...
void Compute_SSSP(graph g,int src,int* weight){
  ...
  while (is_finished (startv,endv,modified) )
  {
    ...
    for (int v = startv; v <= endv; v ++) 
    {
        ...
        {
            if(temp != my_rank)
              MPI_Isend(send_data+temp*3*max_degree, 1, MPI_INT, temp, 0, MPI_COMM_WORLD, &request);
              if(send_data[temp*3*max_degree] > 0)
                MPI_Isend(send_data+(temp*3*max_degree+1), send_data[temp*3*max_degree]*3, MPI_INT, temp, 1, MPI_COMM_WORLD, &request);
        }
        MPI_Barrier(MPI_COMM_WORLD);
        */
        ...
        for(int t=0;t<np;t++)
        {
            if(t != my_rank)
            {
                ...
                if(amount>0)
                {
                    ...
                    MPI_Recv(buffer, (amount*3), MPI_INT, t, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    //MPI_Get(buffer, (amount*4), MPI_INT, t, (my_rank*4*max_degree+1), (amount*4), MPI_INT, window);
                    for(int k=0;k<(amount*3);k+=3)
                    {
