#include "sssp_sparse_boost_v2.h" 
void Compute_SSSP(graph g,int src){
  ...
  bool* modified_next=new bool[g.num_nodes()];
            ...
            else
            {
              ...
              itr = send_data[dest_pro].find(nbr);
     	        if (itr != send_data[dest_pro].end())
                  itr->second = min(send_data[dest_pro][nbr],dist[v]+wt);
              else
                send_data[dest_pro][nbr] = dist[v] + wt;
              /*
              dest_pro = nbr / part_size;
   ...
   */
    ...
    for(int t=0;t<np;t++)
    {
      if(t != my_rank)
      {
            ...
            {
                int nbr = x.first; int dist_v = x.second;
                if (dist[nbr] > dist_v )
                        {
                            dist[nbr] = dist_v;
                            //modified_next.push_back(nbr);
       ...
       // MPI_Recv(&buffer[0], amount, MPI_INT, t, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE); 
        ...
        for(int v=startv;v<=endv;v++)
        {
        	modified[v] = modified_next[v];
        	modified_next[v] = false;
        }
  ...
  gather(world, dist+my_rank*part_size,part_size,final_dist,0);
  //gather(world, dist+my_rank*part_size,part_size,0);
  if(my_rank==0)
  {
