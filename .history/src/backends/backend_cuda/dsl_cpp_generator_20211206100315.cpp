...
{  
   ...
   if(stmt->getTypeofNode()==NODE_ASSIGN)
     { 
       ...
       generateAssignmentStmt((assignment*)stmt,isHeader); //Change "true" to later ; currently one asst in forall/kernel
     }
...
void dsl_cpp_generator::generateAssignmentStmt(assignment* asmt, bool isKernel=false)
{  
   
   dslCodePad &targetFile = isKernel ? header : main;
   ...
   else if(asmt->lhs_isProp())  //the check for node and edge property to be carried out.
   {
     ...
     if(isKernel)targetFile.pushString("d_");   //assuming only LHS assingment vars are device vars. Eg. dv[i] =dv[i]+3; may not work. for now ok!	
     header.pushString(propId->getIdentifier2()->getIdentifier());	
     header.push('[');	
     header.pushString(propId->getIdentifier1()->getIdentifier());	
     header.push(']');	
     std::cout<< propId->getIdentifier2()->getIdentifier() << " " << propId->getIdentifier1()->getIdentifier() << '\n';
...
void dsl_cpp_generator::generateForAllSignature(forallStmt* forAll)
{
  ...
  if(forAll->isSourceProcCall())
  {
    ...
    if(allGraphIteration(iteratorMethodId->getIdentifier()))
    {
      ...
      generateLaunchConfig(methodId);
      if(s.compare("nodes")==0)
      {

        cout<<"INSIDE NODES VALUE"<<"\n";
        std::cout<< "LAUNCH CONFIF WITH NODES" << '\n';

        //~ generateForKernel("");
        //~ kernelsampleFor<<<numBlocks,threadsPerBlock>>>(V,d_meta,d_data,d_weight,d_nodeVal);
                                                 //~                //GRAPH g //PARAM+DEFINED IN DSL
        //~ generateCudaDeviceSync();

        //~ sprintf(strBuffer,"kernelsampleFor<<<numBlocks,threadsPerBlock>>>(V,d_meta,d_data,d_weight,d_nodeVal)");
        strcpy(strBuffer, "kernelsampleFor<<<numBlocks,threadsPerBlock>>>(");
        main.pushString(strBuffer);

        int ii=0;
        for (auto v : varList){
         sprintf(strBuffer,"%s",v.varName.c_str());
          if(ii++ < varList.size()-1)   // HANDLE "," after each parameter except last
            strcat(strBuffer,", ") ;
          main.pushString(strBuffer); // WRITE TO HEADER
        }
