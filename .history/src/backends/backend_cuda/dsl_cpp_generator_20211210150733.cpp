...
{  
   if(stmt->getTypeofNode()==NODE_BLOCKSTMT)
     {
       generateBlock((blockStatement*)stmt, true);

     }
   if(stmt->getTypeofNode()==NODE_DECL)
   {
     
      generateVariableDecl((declaration*)stmt, true);
   ...
   if(stmt->getTypeofNode()==NODE_ASSIGN)
     { 
      ...
      generateAssignmentStmt((assignment*)stmt,true); //Change "true" to later ; currently one asst in forall/kernel
     }
   ...
   {
      generateIfStmt((ifStmt*)stmt, true);
   }

   if(stmt->getTypeofNode()==NODE_DOWHILESTMT)
   {
      generateDoWhileStmt((dowhileStmt*) stmt, true);
   }

    if(stmt->getTypeofNode()==NODE_FORALLSTMT)
     {
       generateForAll((forallStmt*) stmt, true);
     }
  
    if(stmt->getTypeofNode()==NODE_FIXEDPTSTMT)
    {
      generateFixedPoint((fixedPointStmt*)stmt, true);
    }
    if(stmt->getTypeofNode()==NODE_REDUCTIONCALLSTMT)
    { cout<<"IS REDUCTION STMT HI"<<"\n";
      generateReductionStmt((reductionCallStmt*) stmt, true);
    }
    ...
    { 
      generateProcCall((proc_callStmt*) stmt, true);
    }
    if(stmt->getTypeofNode()==NODE_UNARYSTMT)
    {
        unary_stmt* unaryStmt=(unary_stmt*)stmt;
        generateExpr(unaryStmt->getUnaryExpr(), true);
        main.pushstr_newL(";");
...
{  
   ...
   for(itr=stmtList.begin();itr!=stmtList.end();itr++)
   {
     ...
     generateStatement(stmt, targetFile);
