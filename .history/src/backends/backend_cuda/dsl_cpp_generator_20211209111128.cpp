   ...
   if(collectionId->getSymbolInfo()->getType()->gettypeId()==TYPE_SETN)
     {
      ...
      if(body->getTypeofNode()==NODE_BLOCKSTMT)
      {
        generateBlock((blockStatement*)body,false,false);
        main.pushstr_newL("}");
      }
      else
         generateStatement(forAll->getBody());
...
{ char strBuffer[1024];
  ...
  generateStatement(fixedPointConstruct->getBody());
    ...
    generateBlock((blockStatement*)fixedPointConstruct->getBody(),false,true);
...
{  char strBuffer[1024];
   ...
   main.pushstr_newL("{");
   generateBlock(func->getBlockStatement(),false,true);
   main.NewLine();
   main.push('}');
