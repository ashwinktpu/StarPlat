...
spmpi::reduction_details* red_details = NULL;
spmpi::make_par* mp = NULL;
...
void dsl_cpp_generator::generateInnerReductionStmt(reductionCallStmt* stmt,int send)
{ char strBuffer[1024];
  //if(stmt->getType() == 5)
  if(stmt->is_reducCall())
  {    
...
void dsl_cpp_generator::generateReductionStmtForSend(reductionCallStmt* stmt,bool send)
{ char strBuffer[1024];
  //if(stmt->getType() == 5)
  if(stmt->is_reducCall())
      {
...
void dsl_cpp_generator::generateExprForSend(Expression* expr,int send, Identifier* remote,Identifier* replace)
{
...
void dsl_cpp_generator::generateProcCallForSend(Expression* expr,int send, Identifier* remote, Identifier* replace)
{
    char strBuffer[1024];
...
void dsl_cpp_generator::generateAssignmentForSend(assignment* stmt,int send, Identifier* remote, Identifier* replace)
{
...
void dsl_cpp_generator::generateIdentifierForSend(Identifier* stmt,int send, Identifier* remote,Identifier* replace)
{
  if(send == 3)
  {
    Identifier* id = stmt;
...
void dsl_cpp_generator::generatePropAccessForSend(PropAccess* stmt,int send, Identifier* remote,Identifier* replace)
{
  ...
  cout<<"Testing........................."<< i1  << " " << i2 << " " <<  rem << " " << send << endl;
  if(i1 == rem)
  {
    if(send == 1)
    {
      ...
      std::cout << "exiting send 1" << std::endl;
    }
    else if(send == 3)
    {
...
void dsl_cpp_generator::generateArLForSend(Expression* stmt,int send, Identifier* remote,Identifier* replace)
{
...
void dsl_cpp_generator::generate_exprLiteralForSend(Expression* expr,int send,Identifier *remote,Identifier* replace)
     {  
...
void dsl_cpp_generator::generate_sendCall(statement* body)
{
    char strBuffer[1024];
...
void dsl_cpp_generator::generate_receiveCall(statement* body)
{
    main.pushstr_newL("for(int t=0;t<np;t++)");
    main.pushstr_newL("{");
        main.pushstr_newL("if(t != my_rank)");
...
void dsl_cpp_generator::generate_receiveCallBFS(statement* body,int send,Identifier* remote)
{
...
void dsl_cpp_generator::generate_addMessage(statement* stmt,int send,Identifier* remote, Identifier* replace)
{
  char strBuffer[1024];
  if(stmt->getTypeofNode() == NODE_BLOCKSTMT)
  {
...
void dsl_cpp_generator::generateReceiveBlock(blockStatement* body)
{
...
void dsl_cpp_generator::generateForAllFilter(forallStmt* forAll)
{
   proc_callExpr* extractElemFunc=forAll->getExtractElementFunc();
   PropAccess* sourceField=forAll->getPropSource();
   Identifier* iterator=forAll->getIterator();
...
void dsl_cpp_generator::generateForSignature(forallStmt* forAll)
{
...
void dsl_cpp_generator::generateFor(forallStmt* forAll)
{ 
...
void dsl_cpp_generator:: generateDoWhileStmt(dowhileStmt* stmt)
{
   char strBuffer[1024];
...
void dsl_cpp_generator::generate_exprArLReceive(Expression* expr)
{
  //cout<<"INSIDE RELATIONAL EXPR"<<"\n";
...
void dsl_cpp_generator::generateCheckIfNeighbor()
{
  if(!is_an_edge)
  {
...
//***************Function to translate property id***************//
void dsl_cpp_generator::generate_exprPropId_Pull(PropAccess* propId, Identifier* replace_id) //This needs to be made more generic.
{ char strBuffer[1024];
  //PropAccess* propId=(PropAccess*)expr->getPropId();
  Identifier* id1=propId->getIdentifier1();
  Identifier* id2=propId->getIdentifier2();
...
void dsl_cpp_generator::generate_exprPropIdReceive(PropAccess* propId) //This needs to be made more generic.
{ char strBuffer[1024];
  //PropAccess* propId=(PropAccess*)expr->getPropId();
  Identifier* id1=propId->getIdentifier1();
  Identifier* id2=propId->getIdentifier2();
...
{  
    ...
    */
    ...
    std::cout << "func count" << curFuncCount() << std::endl;
    std::cout << "Graph var: " << graphVar[0]->getIdentifier() << std::endl;
    ...
    if(comm_needed) {
      ...
      sprintf(strBuffer,"%s.parseGraph();",graphVar[0]->getIdentifier());
      main.pushstr_newL(strBuffer);

      sprintf(strBuffer,"_num_nodes = %s.num_nodes();",graphVar[0]->getIdentifier());
      main.pushstr_newL(strBuffer);
      sprintf(strBuffer,"_actual_num_nodes = %s.ori_num_nodes();",graphVar[0]->getIdentifier());
      main.pushstr_newL(strBuffer);
      sprintf(strBuffer,"all_weight = %s.getEdgeLen();",graphVar[0]->getIdentifier());
      main.pushstr_newL(strBuffer);
      sprintf(strBuffer,"edgeList = %s.getEdgeList();",graphVar[0]->getIdentifier());
      main.pushstr_newL(strBuffer);
      sprintf(strBuffer,"srcList = %s.getSrcList();",graphVar[0]->getIdentifier());
      main.pushstr_newL(strBuffer);
      sprintf(strBuffer,"index = %s.getIndexofNodes();",graphVar[0]->getIdentifier());
      main.pushstr_newL(strBuffer);
      sprintf(strBuffer,"rev_index = %s.getRevIndexofNodes();",graphVar[0]->getIdentifier());
      main.pushstr_newL(strBuffer);
      sprintf(strBuffer,"part_size = %s.num_nodes()/np;",graphVar[0]->getIdentifier());
      main.pushstr_newL(strBuffer);
      ...
      main.pushstr_newL("}");
      sprintf(strBuffer,"int max_degree = %s.%s();",graphVar[0]->getIdentifier(),"max_degree");
      main.pushstr_newL(strBuffer);
      sprintf(strBuffer,"int *weight = %s.getEdgeLen();",graphVar[0]->getIdentifier());
      main.pushstr_newL(strBuffer);
      //main.pushstr_newL("int* weight=g.getEdgeLen();");
      main.pushstr_newL("");

      sprintf(strBuffer,"part_size = %s.%s()/np;",graphVar[0]->getIdentifier(),"num_nodes");
      main.pushstr_newL(strBuffer);
      main.pushstr_newL("startv = my_rank*part_size;");
      main.pushstr_newL("endv = startv + (part_size-1);");
      main.pushstr_newL("");
   ...
   std::cout << "Before generating function body" << std::endl;
