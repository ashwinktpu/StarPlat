...
void dsl_cpp_generator::addProfilling(const char *eventName) {
  ...
  main.pushstr_newL(strBuffer);
  sprintf(strBuffer,
          "status = clGetEventProfilingInfo(%s, CL_PROFILING_COMMAND_START, "
          "sizeof(cl_ulong), &start, NULL);",
          eventName);
  main.pushstr_newL(strBuffer);
  sprintf(strBuffer,
          "status = clGetEventProfilingInfo(%s, CL_PROFILING_COMMAND_END, "
...
void dsl_cpp_generator::generateGetPlatforms() {
  ...
  main.pushstr_newL("platforms = (cl_platform_id "
                    "*)malloc(number_of_platforms*sizeof(cl_platform_id));");
  main.pushstr_newL(
...
void dsl_cpp_generator::generateGetDevices() {
  ...
  main.pushstr_newL("status = clGetDeviceIDs(platforms[0],CL_DEVICE_TYPE_GPU, "
                    "0, NULL, &number_of_devices);");
  main.pushstr_newL("devices = (cl_device_id "
                    "*)malloc(number_of_devices*sizeof(cl_device_id));");
  main.pushstr_newL("status = clGetDeviceIDs(platforms[0], CL_DEVICE_TYPE_GPU, "
                    "number_of_devices, devices, NULL);");
...
void dsl_cpp_generator::generateCreateContext() {
  ...
  main.pushstr_newL("context = clCreateContext(NULL, number_of_devices, "
...
void dsl_cpp_generator::generateCreateCommandQueue() {
  ...
  main.pushstr_newL("command_queue = clCreateCommandQueue(context, devices[0], "
...
void dsl_cpp_generator::generateCudaMallocStr(const char *dVar,
  ...
  char strBuffer[1024];
  // cl_mem vertices_device = clCreateBuffer(context, CL_MEM_READ_ONLY,
  // (n+1)*sizeof(int), NULL, &status);
  sprintf(strBuffer,
          "cl_mem %s = clCreateBuffer(context, CL_MEM_READ_WRITE, "
          "%s*sizeof(%s), NULL, &status);",
          dVar, sizeOfType, typeStr);
  main.pushstr_newL(strBuffer);
...
*/
  ...
  if (isH2D) {
    sprintf(strBuffer,
            "status = clEnqueueWriteBuffer(command_queue, %8s , CL_TRUE, 0, "
            "sizeof(%3s)*%s, %8s, 0, NULL, NULL );",
            sVarName, type, sizeV, tVarName);
  } else {
    sprintf(strBuffer,
            "clEnqueueReadBuffer(command_queue, %s , CL_TRUE, 0, "
            "sizeof(%s)*%s, %s, 0, NULL, NULL );",
            tVarName, type, sizeV, sVarName);
  }
  main.pushstr_newL(strBuffer);
...
void dsl_cpp_generator::generateCudaMalloc(Type *type, const char *identifier) {
  ...
  sprintf(
      strBuffer,
      "cl_mem d_%s = "
      "clCreateBuffer(context,CL_MEM_READ_WRITE,(%s)*sizeof(%s),NULL, "
      "&status);",
      identifier, (type->isPropNodeType()) ? "V" : "E",
      convertToCppType(type->getInnerTargetType())); // this assumes PropNode
                                                     // type  IS PROPNODE? V : E
                                                     // //else might error later
...
void dsl_cpp_generator::generateFuncBody(Function *proc, int isMainFile) {
  ...
  if (genCSR) {
    ...
    // generateLaunchConfig("nodes");  // Most of the time unless we perform
    // edgeBased!
...
void dsl_cpp_generator::generateVariableDecl(declaration *declStmt,
  ...
  if (type->isPropType()) {
    if (type->getInnerTargetType()->isPrimitiveType()) {
      ...
      char strBuffer[1024];
      sprintf(strBuffer, "%s *h_%s = (%s *)malloc(%s*sizeof(%s));",
              convertToCppType(innerType),
              declStmt->getdeclId()->getIdentifier(),
              convertToCppType(innerType), type->isPropNodeType() ? "V" : "E",
              convertToCppType(innerType));
      main.pushString(strBuffer);
      main.NewLine();
  ...
  // needs to handle carefully for PR code generation
    ...
    if (isMainFile == 1) {
      ...
      sprintf(strBuffer,
              "cl_mem d_%s= clCreateBuffer(context, CL_MEM_READ_WRITE, "
              "sizeof(%s), NULL, &status);",
              varName, varType);
      main.pushstr_newL(strBuffer);
    }
...
void dsl_cpp_generator::generate_exprLiteral(Expression *expr, int isMainFile) {
  dslCodePad &targetFile = getTargetFile(isMainFile);
  char valBuffer[1024];
    ...
    PropAccess *propId, int isMainFile) // This needs to be made more generic.
{
  ...
  if (id2->getSymbolInfo() != NULL &&
    ...
    if (isMainFile == 2)
      sprintf(strBuffer, "d_%s[%s]", id2->getIdentifier(),
              id1->getIdentifier()); // PREFIX D
    else
...
void dsl_cpp_generator::generate_exprArL(
...
void dsl_cpp_generator::generate_exprProcCall(Expression *_expr,
  ...
  } else if (methodId == "merge") {

    list<argument*> arglist = proc->getArgList();
    assert(arglist.size() == 2);
    Identifier *dsu_id = proc->getId1();
    Identifier *elem1_id = arglist.front()->getExpr()->getId();
    Identifier *elem2_id = arglist.back()->getExpr()->getId();
    assert(dsu_id);
    assert(elem1_id);
    assert(elem2_id);
...
void dsl_cpp_generator::generateExpr(
  ...
  if (expr->isLiteral()) {
    ...
    if (expr->getId()) std::cout << "\tid: " << expr->getId()->getIdentifier()
                                 << "\n";
    assert(false);
  }
}
...
/**
 * Generate statement for procedure call
 * TODO make this work with generate_exprProcCall()
 */
void dsl_cpp_generator::generateProcCall(proc_callStmt *proc_callStmt,
                                         int isMainFile) {
  std::cout << "generating statement for proc call\n";
...
void dsl_cpp_generator::generateInitkernel1(assignment *assign,
                                            int isMainFile) {
  ...
  if (!isKenelPresent(kernelName)) {
    ...
    sprintf(strBuffer, "__kernel void %s( __global %s *d_%s , %s value, int v",
            kernelName, inVarType, inVarName, inVarType);
    ...
    main.NewLine();
    sprintf(strBuffer,
            "cl_kernel %s = clCreateKernel(program, \"%s\", &status);",
  ...
  main.NewLine();
  sprintf(strBuffer,
          "status = clSetKernelArg(%s, 0 , sizeof(cl_mem), (void *)& d_%s);",
          kernelName, inVarName);
  main.pushString(strBuffer);
  main.NewLine();
  sprintf(strBuffer,
          "status = clSetKernelArg(%s, 1, sizeof(%s) , (void*)& %sValue);",
          kernelName, inVarType, inVarName);
  main.pushString(strBuffer);
  main.NewLine();
  sprintf(strBuffer, "status = clSetKernelArg(%s, 2, sizeof(int), (void*)&V);",
          kernelName);
  main.pushString(strBuffer);
  main.NewLine();
  // Launch the kernel from main
  main.pushstr_newL("local_size = 128;");
  sprintf(strBuffer, "global_size = %s;",
          inId->getSymbolInfo()->getType()->isPropNodeType()
              ? "(V + local_size -1)/ local_size * local_size"
              : "(E + local_size -1)/ local_size * local_size");
  main.pushstr_newL(strBuffer);

  sprintf(strBuffer,
          "status = clEnqueueNDRangeKernel(command_queue, %s, 1, NULL, "
...
void dsl_cpp_generator::generateCudaMallocParams(
  ...
  for (itr = paramList.begin(); itr != paramList.end(); itr++) {
    ...
    if (type->isPropType()) {
      if (type->getInnerTargetType()->isPrimitiveType()) {
        ...
        sprintf(strBuffer,"%s *%s =(%s *)malloc(%s*sizeof(%s));",cpptype,
        varName,cpptype,size,cpptype); main.pushstr_newL(strBuffer);
        */
        generateCudaMalloc(type, (*itr)->getIdentifier()->getIdentifier());
      }
    }
    if (type->isPrimitiveType()) // for the premittive type just declare those
                                 // variables in device memory
    {
      // Type* innerType = type->getInnerTargetType();
      Identifier *iden = (*itr)->getIdentifier();
      sprintf(strBuffer, "%s *h_%s;", convertToCppType(type),
              iden->getIdentifier());
      main.pushstr_newL(strBuffer);
      sprintf(
          strBuffer,
          "cl_mem d_%s = "
          "clCreateBuffer(context,CL_MEM_READ_WRITE,sizeof(%s),NULL, &status);",
...
void dsl_cpp_generator::generateDeviceAssignmentStmt(assignment *asmt,
                                 ...
                                 // // carried out.
  {
    ...
    if (asmt->isDeviceAssignment()) {
      ...
      sprintf(strBuffer, "//Index%s %s initialization",
              propId->getIdentifier2()->getIdentifier(),
              propId->getIdentifier1()->getIdentifier());
      main.pushstr_newL(strBuffer);
      ...
      main.pushstr_newL(strBuffer);
      sprintf(strBuffer,
              "status = clSetKernelArg(%s, 1,sizeof(int), (void*)&%s);",
              kernelName, propId->getIdentifier1()->getIdentifier());
      main.pushstr_newL(strBuffer);
      sprintf(
          strBuffer,
          "status = clSetKernelArg(%s, 2, sizeof(%s),(void *)&init%sval%s);",
          kernelName, varTypeB, propId->getIdentifier2()->getIdentifier(),
          propId->getIdentifier1()->getIdentifier());
      main.pushstr_newL(strBuffer);
...
void dsl_cpp_generator::checkForAllAndGenerate(blockStatement *blockStmt,
                                               int isMainFile) {
  dslCodePad &targetFile = getTargetFile(isMainFile);
    ...
    if (stmt->getTypeofNode() == NODE_FORALLSTMT) {
      ...
      if (forAll->isForall()) {
        ...
        if (!isKenelPresent(kernelName)) {
          sprintf(
              strBuffer,
              "cl_kernel %s = clCreateKernel(program,\"%s_kernel\",&status);",
              kernelName, kernelName);
          targetFile.pushString(strBuffer);
          targetFile.NewLine();
          addKernelObject(kernelName);
        ...
        sprintf(strBuffer,
                "status = clSetKernelArg(%s, 0, sizeof(int) , (void*)&V);",
                kernelName);
        targetFile.pushString(strBuffer);
        targetFile.NewLine();
        sprintf(strBuffer,
                "status = clSetKernelArg(%s, 1, sizeof(int) , (void*)&E);",
                kernelName);
        targetFile.pushString(strBuffer);
        targetFile.NewLine();
        sprintf(
            strBuffer,
            "status = clSetKernelArg(%s, 2, sizeof(cl_mem) , (void*)&d_meta);",
            kernelName);
        targetFile.pushString(strBuffer);
        targetFile.NewLine();
        sprintf(
            strBuffer,
            "status = clSetKernelArg(%s, 3, sizeof(cl_mem) , (void*)&d_data);",
            kernelName);
        targetFile.pushString(strBuffer);
        targetFile.NewLine();
        sprintf(
            strBuffer,
            "status = clSetKernelArg(%s, 4, sizeof(cl_mem) , (void*)&d_src);",
            kernelName);
        targetFile.pushString(strBuffer);
        targetFile.NewLine();
        sprintf(strBuffer,
                "status = clSetKernelArg(%s, 5, sizeof(cl_mem) , "
                "(void*)&d_weight);",
                kernelName);
        targetFile.pushString(strBuffer);
        targetFile.NewLine();
        sprintf(strBuffer,
                "status = clSetKernelArg(%s, 6, sizeof(cl_mem) , "
                "(void*)&d_rev_meta);",
                kernelName);
        targetFile.pushString(strBuffer);
        targetFile.NewLine();
        sprintf(strBuffer,
                "status = clSetKernelArg(%s, 7, sizeof(cl_mem) , "
                "(void*)&d_modified_next);",
                kernelName);
        targetFile.pushString(strBuffer);
        targetFile.NewLine();
        ...
        int argCount = 8;
        // If parent is fixedpoint Node then get the fixpoint variable and add
        // it to the kernel that means generate clSetKernelArg for that fixPoint
        // variable
        ...
        if (p->getParent()->getTypeofNode() == NODE_FIXEDPTSTMT) {
          ...
          sprintf(strBuffer,
                  "status = clSetKernelArg(%s, %d , sizeof(cl_mem), "
                  "(void*)&d_finished);",
                  kernelName, argCount);
          targetFile.pushstr_newL(strBuffer);
          argCount++;
...
void dsl_cpp_generator::generateFixedPoint(fixedPointStmt *fixedPointConstruct,
  ...
  const char *fixPointVarType =
      convertToCppType(fixedPointId->getSymbolInfo()->getType());
  ...
  if (dependentId != NULL) {
    ...
    if (dependentId->getSymbolInfo()->getType()->isPropType()) {
      if (dependentId->getSymbolInfo()->getType()->isPropNodeType()) {
        ...
        targetFile.NewLine();
        /*
        // create clmem corresponding to fixPointVar in main codepad and to
        mapping/implement pinned memory sprintf(strBuffer,"d_%s =
        clCreateBuffer(context,
        CL_MEM_READ_WRITE|CL_MEM_ALLOC_HOST_PTR,sizeof(int), NULL, &status);",
        fixPointVar); main.NewLine(); main.pushString(strBuffer);
        ...
        addMemObject(str);
        main.NewLine();
        sprintf(strBuffer, "h_%s = (%s *)clEnqueueMapBuffer(command_queue, d_%s
        , CL_TRUE, CL_MAP_READ|CL_MAP_WRITE,
        0,sizeof(%s),0,NULL,NULL,&status);", fixPointVar, fixPointVarType,
        fixPointVar,fixPointVarType); main.pushString(strBuffer);
        main.NewLine();
        */
        // initailize modified_next
        // Create kernel for modified_next
        // Add a if condition Here, What if I have created a kernel for this
        // already Don't want to repeat
        char *kernelName = (char *)malloc(100 * sizeof(char));
        strcpy(kernelName, "init");
        ...
        if (!isKenelPresent(kernelName)) {
          addKernelObject(kernelName);
          sprintf(strBuffer,
                  "__kernel void %s_kernel(__global %s *%s, %s val ,  int v){",
                  kernelName, fixPointVarType, modifiedVarNext,
                  fixPointVarType);
          kernel.pushString(strBuffer);
          kernel.NewLine();
        ...
        sprintf(strBuffer,
                "status = clSetKernelArg(init%s, 0,sizeof(cl_mem),(void*)&%s);",
                modifiedVarNext, modifiedVarNext);
        main.pushString(strBuffer);
        main.NewLine();
        sprintf(strBuffer, "%s %sValue = false;", fixPointVarType,
                modifiedVarNext);
        main.pushString(strBuffer);
        main.NewLine();
        sprintf(
            strBuffer,
            "status = clSetKernelArg(init%s, 1,sizeof(%s) ,(void*)&%sValue);",
            modifiedVarNext, fixPointVarType, modifiedVarNext);
        main.pushString(strBuffer);
        main.NewLine();
        sprintf(strBuffer,
                "status = clSetKernelArg(init%s, 2 , sizeof(int),(void*)&V);",
                modifiedVarNext);
        main.pushString(strBuffer);
        main.NewLine();
        ...
        main.NewLine();
        sprintf(strBuffer,
                "status = clEnqueueNDRangeKernel(command_queue, init%s , 1, "
                "NULL , &global_size , &local_size ,0, NULL, &event);",
                modifiedVarNext);
        main.pushString(strBuffer);
        main.NewLine();
        addProfilling("event");
        sprintf(strBuffer, "status = clReleaseKernel(init%s);",
                modifiedVarNext);
        main.pushstr_newL(strBuffer);
        ...
        targetFile.pushString(strBuffer);
        targetFile.NewLine();
        // Line below assumes there is a kernel call whithin fixedPoint
        // construct
        sprintf(strBuffer,
                " status  = clEnqueueWriteBuffer(command_queue,d_%s, CL_TRUE, "
                "0,sizeof(int), &%s,0,0,NULL);",
                fixPointVar, fixPointVar);
        targetFile.pushstr_newL(strBuffer);
        targetFile.NewLine();
        // Check for ForAll statement if yes then generate create kernel and set
        // kernel argument
        ...
        sprintf(
            strBuffer,
            "status = clEnqueueCopyBuffer(command_queue, %s ,d_%s, 0,0, "
            "%s*sizeof(%s),0,NULL, NULL);",
            modifiedVarNext, modifiedVar,
            dependentId->getSymbolInfo()->getType()->isPropNodeType() ? "V"
                                                                      : "E",
            convertToCppType(
                dependentId->getSymbolInfo()->getType()->getInnerTargetType()));
        targetFile.pushString(strBuffer);
        targetFile.NewLine();
        // need to create kernel in main file
        main.pushstr_newL("// creating %s kernel");
        ...
        sprintf(strBuffer,
                "status = clEnqueueNDRangeKernel(command_queue, init%s , "
                "1,NULL, &global_size , &local_size ,0, NULL, &event);",
                modifiedVarNext);
        targetFile.pushString(strBuffer);
        targetFile.NewLine();
        addProfilling("event");
        ...
        sprintf(strBuffer,
                "status =clEnqueueReadBuffer(command_queue, d_%s , CL_TRUE, 0, "
        ...
        targetFile.NewLine();
        sprintf(strBuffer,
                "clEnqueueUnmapMemObject(command_queue, d_%s, h_%s, 0, NULL, "
                "NULL);",
                fixPointVar, fixPointVar);
        targetFile.pushString(strBuffer);
        targetFile.NewLine();
...
void dsl_cpp_generator::generateCudaMemCpySymbol(char *var, const char *typeStr,
                                                 bool direction) {
  ...
  if (direction) {
    main.NewLine();
    sprintf(strBuffer,
            "status = clEnqueueWriteBuffer(command_queue, d_%s, CL_TRUE, 0, "
            "sizeof(%s),&%s , 0, NULL, NULL);",
            var, typeStr, var);
    main.pushString(strBuffer);
    main.NewLine();
  } else {
...
void dsl_cpp_generator::addCudaKernel(forallStmt *forAll) {
  ...
  kernel.pushString("(int V,  int E, __global int* d_meta, __global int* "
                    "d_data, __global int* d_src,");
  kernel.NewLine();
    ...
    if (parent->getTypeofNode() == NODE_FIXEDPTSTMT) {
      ...
      if (convergeExpr->getExpressionFamily() == EXPR_UNARY) {
        if (convergeExpr->getUnaryExpr()->getExpressionFamily() == EXPR_ID) {
          ...
          sprintf(strBuffer, ",__global %s d_%s_next",
                  convertToCppType(dependentId->getSymbolInfo()->getType()),
  ...
  for (Identifier *iden : vars) {
    ...
    Type *type = iden->getSymbolInfo()->getType();
    if (type->isPropType()) {
      sprintf(strBuffer, ",__global %s d_%s", convertToCppType(type),
              iden->getIdentifier());
      kernel.pushString(/*createParamName(*/ strBuffer);
    } else if (type->isPrimitiveType()) {
      sprintf(strBuffer, ",__global %s* %s", convertToCppType(type),
              iden->getIdentifier());
      kernel.pushString(/*createParamName(*/ strBuffer);
  ...
  if (forAll->hasFilterExpr()) {
    ...
    cout << "============CHANGED BODY  TYPE=============="
         << (changedBody->getTypeofNode() == NODE_BLOCKSTMT);
  ...
  for (statement *stmt : statementList) {
    generateStatement(
        stmt, 2); // 2 means in kernelFile. All these stmts should be inside
                  // kernel ~ if (stmt->getTypeofNode() == NODE_FORALLSTMT) {
  }
  ...
  if (forAll->isSourceProcCall()) {
    ...
    if (allGraphIteration(iteratorMethodId->getIdentifier())) {
      ...
      if (s.compare("neighbors") == 0) {
        ...
        //~ sprintf(strBuffer,"for (int edge = d_meta[v]; %s < %s[%s+1]; %s++) {
        //// ","int","edge","d_meta","v","edge","d_meta","v","edge");
        sprintf(strBuffer,
                "for (%s %s = %s[%s]; %s < %s[%s+1]; %s++) { // FOR NBR ITR ",
                "int", "edge", "d_meta", "v", "edge", "d_meta", "v", "edge");
        targetFile.pushstr_newL(strBuffer);
        //~ targetFile.pushString("{");
        sprintf(strBuffer, "%s %s = %s[%s];", "int", iterator->getIdentifier(),
                "d_data", "edge"); // needs to move the addition of
        targetFile.pushstr_newL(strBuffer);
      }
      if (s.compare("nodes_to") == 0) // for pageRank
      {
        ...
        assert(argList.size() == 1);
        Identifier *nodeNbr = argList.front()->getExpr()->getId();
        sprintf(strBuffer, "for (%s %s = %s[%s]; %s < %s[%s+1]; %s++)", "int",
                "edge", "d_rev_meta", nodeNbr->getIdentifier(), "edge",
                "d_rev_meta", nodeNbr->getIdentifier(), "edge");
        targetFile.pushstr_newL(strBuffer);
        targetFile.pushstr_newL("{");
        sprintf(strBuffer, "%s %s = %s[%s] ;", "int", iterator->getIdentifier(),
                "d_src", "edge"); // needs to move the addition of
        targetFile.pushstr_newL(strBuffer);
    ...
    if (sourceId != NULL) {
      if (sourceId->getSymbolInfo()->getType()->gettypeId() ==
          TYPE_SETN) { // FOR SET
        //~ std::cout << "+++++     ++++++++++" << '\n';
        main.pushstr_newL(
...
void dsl_cpp_generator::generateForAll(forallStmt *forAll, int isMainFile) {
  ...
  if (forAll->isForall()) // IS FORALL
  {
    ...
    // If there is any variable of premitive data type then need to do transfer
    // it to device
    for (Identifier *iden : vars) {
      ...
      if (iden->getSymbolInfo() == NULL)
        continue; // for (v in g.nodes()) for 'v' I didn't find an enty in table
      Type *type = iden->getSymbolInfo()->getType();

      if (type->isPrimitiveType())
        generateCudaMemCpySymbol(iden->getIdentifier(), convertToCppType(type),
                                 true);
    ...
    int argCount = 0;
    sprintf(strBuffer,
            "status = clSetKernelArg(%s,%d, sizeof(int),(void *)&V);",
            KernelName, argCount++);
    main.pushstr_newL(strBuffer);
    sprintf(strBuffer,
            "status = clSetKernelArg(%s,%d, sizeof(int),(void *)&E);",
            KernelName, argCount++);
    main.pushstr_newL(strBuffer);
    sprintf(strBuffer,
            "status = clSetKernelArg(%s,%d, sizeof(cl_mem),(void *)&d_meta);",
            KernelName, argCount++);
    main.pushstr_newL(strBuffer);
    sprintf(strBuffer,
            "status = clSetKernelArg(%s,%d, sizeof(cl_mem),(void *)&d_data);",
            KernelName, argCount++);
    main.pushstr_newL(strBuffer);
    sprintf(strBuffer,
            "status = clSetKernelArg(%s,%d, sizeof(cl_mem),(void *)&d_src);",
            KernelName, argCount++);
    main.pushstr_newL(strBuffer);
    sprintf(strBuffer,
            "status = clSetKernelArg(%s,%d, sizeof(cl_mem),(void *)&d_weight);",
            KernelName, argCount++);
    main.pushstr_newL(strBuffer);
    sprintf(
        strBuffer,
        "status = clSetKernelArg(%s,%d, sizeof(cl_mem),(void *)&d_rev_meta);",
        KernelName, argCount++);
    main.pushstr_newL(strBuffer);
    ...
    if (forAll->getParent()->getParent() != NULL) {
      ...
      if (parent->getTypeofNode() == NODE_FIXEDPTSTMT) {
        ...
        Identifier *fixedPointId = fixedPointConstruct->getFixedPointId();
        sprintf(strBuffer,
                "status = clSetKernelArg(%s,%d, sizeof(cl_mem),(void *)&d_%s);",
                KernelName, argCount++, fixedPointId->getIdentifier());
        main.pushstr_newL(strBuffer);
        assert(convergeExpr->getExpressionFamily() == EXPR_UNARY ||
               convergeExpr->getExpressionFamily() == EXPR_ID);
        ...
        if (convergeExpr->getExpressionFamily() == EXPR_UNARY) {
          if (convergeExpr->getUnaryExpr()->getExpressionFamily() == EXPR_ID) {
            ...
            sprintf(strBuffer,
                    "status = clSetKernelArg(%s,%d, sizeof(cl_mem),(void "
                    "*)&d_%s_next);",
                    KernelName, argCount++, dependentId->getIdentifier());
            main.pushstr_newL(strBuffer);
          }
        }
      }
    ...
    addProfilling("event");
    main.NewLine();
    sprintf(strBuffer, "status = clReleaseKernel(%s);",
    ...
    if (extractElemFunc != NULL) {
      if (neighbourIteration(iteratorMethodId->getIdentifier())) {
        ...
        char *wItr =
            forAll->getIterator()->getIdentifier(); // iterator of for loop
        cout << "Iterator:: " << wItr << endl;
        char *nbrVar;
        if (forAll->getParent()->getParent()->getTypeofNode() ==
            NODE_ITRBFS) // parent is IterateinBFS
        {
          ...
          sprintf(strBuffer, "if(d_level[%s] == (*d_hops_from_source + 1))",
                  wItr);
        ...
        } else if (forAll->getParent()->getParent()->getTypeofNode() ==
                   NODE_ITRRBFS) {
      ...
      if (collectionId->getSymbolInfo()->getType()->gettypeId() ==
          TYPE_SETN) // FOR SET
      {
        if (body->getTypeofNode() == NODE_BLOCKSTMT) {
          targetFile.pushstr_newL("{");
...
void dsl_cpp_generator::generateReductionCallStmt(reductionCallStmt *stmt,
  ...
  if (reduceCall->getReductionType() == REDUCE_MIN) {
    ...
    if (stmt->isListInvolved()) {
      ...
      // cout<< expr->isArithmetic()<<endl; // for SSSP right side expr is
      // Arithmetic
      targetFile.push(';');
      targetFile.NewLine();
      ...
      sprintf(strBuffer, "d_%s[v]!= INT_MAX && ",
...
void dsl_cpp_generator::generateReductionOpStmt(reductionCallStmt *stmt,
  ...
  if (stmt->isLeftIdentifier()) {
    ...
    sprintf(strBuffer, "%s( %s, (%s)", getAtomicFromType(ty, 0),
            id->getIdentifier(), typVar);
    // 0 for atomic Add
    // to implement other tpe of atomics modify getAtomicFromType function and
    // libOpenCL.h we need to implement for min, max, sub etc..
...
void dsl_cpp_generator::generateInitkernelStr(const char *varType,
                                              const char *varName,
                                              const char *value,
                                              const char *sizeVE) {
  char strBuffer[1024];
  sprintf(strBuffer,
          "__kernel void init%s_kernel(__global %s *%s, unsigned int %s)",
          varName, varType, varName, sizeVE);
  kernel.pushstr_newL(strBuffer);
  kernel.pushstr_newL("{");
...
void dsl_cpp_generator::generatePropParams(list<formalParam *> paramList,
                                           char *kernelName,
                                           bool isNeedType = true,
                                           int isMainFile = 1,
                                           int argnum = -1) {
  ...
  for (itr = paramList.begin(); itr != paramList.end(); itr++) {
    ...
    if (type->isPropType()) {
      if (type->getInnerTargetType()->isPrimitiveType()) {
        ...
        strcat(temp2, temp1);
        printf("%s\n", temp2);
        if (isNeedType) // if isNeedType = true we are generating for kernel
                        // else for main
          sprintf(strBuffer, ",__global %s* %s",
                  convertToCppType(type->getInnerTargetType()), temp2);
        else // in main we need argument number for clSetKernelArg
        {
          sprintf(
              strBuffer,
              "status = clSetKernelArg(%s, %d, sizeof(cl_mem), (void*)&%s);",
              kernelName, argnum, temp2);
          // targetFile.pushstr_newL(strBuffer);
        }
...
void dsl_cpp_generator::addCudaBFSIterKernel(iterateBFS *bfsAbstraction) {
  ...
  kernel.pushstr_newL(
      "__kernel void fwd_pass_kernel(int V, __global int* d_meta, __global "
      "int* d_data, __global int* d_weight, __global int* d_level, __global "
      "int* d_hops_from_source, __global int* d_finished");
  ...
  for (auto iden : declaredProp) {
    ...
    sprintf(strBuffer, ",__global %s* d_%s ",
            convertToCppType(
                iden->getSymbolInfo()->getType()->getInnerTargetType()),
            iden->getIdentifier());
    kernel.pushString(strBuffer);
  }
  ...
  char kernelName[2] = ".";
  generatePropParams(
      getCurrentFunc()->getParamList(), kernelName, true, 2,
      9); // true: typeneed 2:kernel file(0:header file, 1: main file)
  kernel.push(')');
  kernel.NewLine();
  kernel.pushstr_newL("{");
  sprintf(strBuffer, "unsigned int %s = get_global_id(0);", loopVar);
  ...
  for (statement *stmt : statementList) {
    generateStatement(stmt,
                      2); // 2:kernel, All these stmts should be inside kernel
  }
...
void dsl_cpp_generator::addCudaRevBFSIterKernel(iterateBFS *bfsAbstraction) {
  ...
  blockStatement *revBlock =
      (blockStatement *)bfsAbstraction->getRBFS()->getBody();
  list<statement *> revStmtList = revBlock->returnStatements();
  const char *loopVar = bfsAbstraction->getIteratorNode()->getIdentifier();
  // backward Pass kernel
  kernel.pushString(
      "__kernel void back_pass_kernel(int V, __global int* d_meta, __global "
      "int* d_data, __global int* d_weight, __global int* d_level, __global "
      "int* d_hops_from_source, __global int* d_finished");
...
void dsl_cpp_generator::addCudaBFSIterationLoop(iterateBFS *bfsAbstraction) {
  ...
  main.pushstr_newL("status  = clEnqueueWriteBuffer(command_queue,d_finished, "
                    "CL_TRUE, 0,sizeof(int), &finished,0,0,NULL);");
  main.pushstr_newL(
      "status  = clEnqueueWriteBuffer(command_queue,d_hops_from_source, "
      "CL_TRUE, 0,sizeof(int), &hops_from_source,0,0,NULL);");

  main.pushstr_newL("//Forward Pass");
  // Create forward pass kernel from program
  // forwardPass(V, d_meta, d_data,d_weight, d_delta, d_sigma, d_level,
  // d_hops_from_source, d_finished,..Other variable..);
  main.pushstr_newL("cl_kernel fwd_pass_kernel = clCreateKernel(program, "
                    "\"fwd_pass_kernel\", &status);");
  main.pushstr_newL("if(status != CL_SUCCESS)");
  main.pushstr_newL("{");
  ...
  main.pushstr_newL("status = clSetKernelArg(fwd_pass_kernel, 1, "
                    "sizeof(cl_mem), (void *)&d_meta);");
  main.pushstr_newL("status = clSetKernelArg(fwd_pass_kernel, 2, "
                    "sizeof(cl_mem), (void *)&d_data);");
  main.pushstr_newL("status = clSetKernelArg(fwd_pass_kernel, 3, "
                    "sizeof(cl_mem), (void *)&d_weight);");
  // delta and sigma are hard coded. what if user uses a different name?
  main.pushstr_newL("status = clSetKernelArg(fwd_pass_kernel, 4, "
                    "sizeof(cl_mem), (void *)&d_level);");
  main.pushstr_newL("status = clSetKernelArg(fwd_pass_kernel, 5, "
                    "sizeof(cl_mem), (void *)&d_hops_from_source);");
  main.pushstr_newL("status = clSetKernelArg(fwd_pass_kernel, 6, "
                    "sizeof(cl_mem), (void *)&d_finished);");
  ...
  for (auto iden : declaredProp) {
    sprintf(strBuffer,
            "status = clSetKernelArg(fwd_pass_kernel, %d, sizeof(cl_mem), "
            "(void*)&d_%s);",
            argNumber, iden->getIdentifier());
    main.pushstr_newL(strBuffer);
    argNumber++;
  }
  ...
  generatePropParams(getCurrentFunc()->getParamList(), kernelName, false, 1,
                     argNumber); // true: typeneed 1:inMainfile
  ...
  main.pushstr_newL("status=clEnqueueReadBuffer(command_queue, d_finished , "
                    "CL_TRUE, 0, sizeof(int), &finished, 0, NULL, NULL );");
  // main.pushstr_newL("status=clEnqueueReadBuffer(command_queue,
  // d_hops_from_source , CL_TRUE, 0, sizeof(int), &hops_from_source, 0, NULL,
  // NULL );");
  main.NewLine();
...
void dsl_cpp_generator::addCudaRevBFSIterationLoop(iterateBFS *bfsAbstraction) {
  ...
  main.pushstr_newL("cl_kernel back_pass_kernel = "
                    "clCreateKernel(program,\"back_pass_kernel\", &status);");
  // set kernel arguments
  // back_pass(V, d_meta, d_data, d_weight, d_delta, d_sigma, d_level,
  // d_hops_from_source, d_finished,...")
  main.pushstr_newL("if(status != CL_SUCCESS)");
  main.pushstr_newL("{");
  ...
  main.pushstr_newL("status = clSetKernelArg(back_pass_kernel, 1, "
                    "sizeof(cl_mem), (void *)&d_meta);");
  main.pushstr_newL("status = clSetKernelArg(back_pass_kernel, 2, "
                    "sizeof(cl_mem), (void *)&d_data);");
  main.pushstr_newL("status = clSetKernelArg(back_pass_kernel, 3, "
                    "sizeof(cl_mem), (void *)&d_weight);");
  main.pushstr_newL("status = clSetKernelArg(back_pass_kernel, 4, "
                    "sizeof(cl_mem), (void *)&d_level);");
  main.pushstr_newL("status = clSetKernelArg(back_pass_kernel, 5, "
                    "sizeof(cl_mem), (void *)&d_hops_from_source);");
  main.pushstr_newL("status = clSetKernelArg(back_pass_kernel, 6, "
                    "sizeof(cl_mem), (void *)&d_finished);");
  ...
  for (auto iden : declaredProp) {
    sprintf(strBuffer,
            "status = clSetKernelArg(back_pass_kernel, %d, sizeof(cl_mem), "
            "(void*)&d_%s);",
            argNumber, iden->getIdentifier());
    main.pushstr_newL(strBuffer);
    argNumber++;
...
void dsl_cpp_generator::generateBFSAbstraction(iterateBFS *bfsAbstraction,
  ...
  main.pushstr_newL(
      "cl_mem d_finished = clCreateBuffer(context, "
      "CL_MEM_READ_WRITE|CL_MEM_ALLOC_HOST_PTR,sizeof(int), NULL, &status);");
  main.pushstr_newL(
      "cl_mem d_hops_from_source = clCreateBuffer(context, "
      "CL_MEM_READ_WRITE|CL_MEM_ALLOC_HOST_PTR, sizeof(int), NULL, &status);");
  main.pushstr_newL("cl_mem d_level = clCreateBuffer(context, "
                    "CL_MEM_READ_WRITE, V*sizeof(int), NULL, &status);");
  // main.pushstr_newL("//map these varibles");
  // main.pushstr_newL("finished = (int *)clEnqueueMapBuffer(command_queue,
  // d_finished , CL_TRUE, CL_MAP_READ|CL_MAP_WRITE,
  // 0,sizeof(int),0,NULL,NULL,&status);"); main.pushstr_newL("hops_from_source
  // = (int *)clEnqueueMapBuffer(command_queue, d_hops_from_source , CL_TRUE,
  // CL_MAP_READ|CL_MAP_WRITE, 0,sizeof(int),0,NULL,NULL,&status);");

  sprintf(strBuffer, "//initialize d_level with -1 and for %s 0",
          bfsAbstraction->getRootNode()->getIdentifier());
  main.pushstr_newL(strBuffer);
  generateInitkernelStr("int", "d_level", "-1", "V");

  // Create kernel->setArgument->runKenrel
  sprintf(strBuffer, "cl_kernel initd_level_kernel = clCreateKernel(program, "
                     "\"initd_level_kernel\", &status);");
  main.pushstr_newL(strBuffer);
  main.pushstr_newL("if(status!=CL_SUCCESS)");
  main.pushstr_newL("{");
  ...
  main.pushstr_newL("}");
  main.pushstr_newL("status = clSetKernelArg(initd_level_kernel , 0, "
                    "sizeof(cl_mem) ,(void *)&d_level);");
  main.pushstr_newL("status = clSetKernelArg(initd_level_kernel , 1, "
                    "sizeof(int) ,(void *)&V);");

  main.pushstr_newL("local_size = 128;");
  main.pushstr_newL(
      "global_size = (V + local_size -1)/ local_size * local_size;");
  main.pushstr_newL("status  = clEnqueueNDRangeKernel(command_queue, "
                    "initd_level_kernel,1,NULL, &global_size, &local_size, 0, "
                    "NULL, &event);");
  addProfilling("event");
  ...
  main.pushstr_newL(
      "cl_kernel initIndexd_level_kernel = "
      "clCreateKernel(program,\"initIndexd_level_kernel\",&status);");

  main.pushstr_newL("status = clSetKernelArg(initIndexd_level_kernel, 0, "
                    "sizeof(cl_mem), (void *)&d_level);");
  sprintf(strBuffer,
          "status = clSetKernelArg(initIndexd_level_kernel, 1, sizeof(int), "
          "(void*)&%s);",
          rootNode);
  main.pushstr_newL(strBuffer);
  ...
  main.pushstr_newL(
      "status = clEnqueueNDRangeKernel(command_queue, initIndexd_level_kernel, "
      "1,NULL, &global_size, &local_size, 0, NULL, &event);");
  addProfilling("event");
  ...
  /*Here we are assuming that if there is iterate in forward BFS
   ...
   So we are directly generating that. Here need some improvement. What if user
   has not written Reverse part? what if user only need forward pass?
   */
  ...
  // main.pushstr_newL("status = clEnqueueUnmapMemObject(command_queue,
  // d_finished, finished, 0, NULL, NULL);"); main.pushstr_newL("status =
  // clEnqueueUnmapMemObject(command_queue, d_hops_from_source,
  // hops_from_source, 0, NULL, NULL);");
...
/**
 * Generate assignment statement for device code.
 * The RHS is generated by a call to generateExpr().
 */
void dsl_cpp_generator::generateAtomicDeviceAssignmentStmt(assignment *asmt,
  ...
  if (asmt->lhs_isIdentifier()) {
    ...
    if (asmt->hasPropCopy()) // prop_copy is of the form (propId = propId)
    {
      ...
      sprintf(strBuffer,
              "status = clEnqueueCopyBuffer(command_queue, d_%s, d_%s, 0, 0, "
              "%s*sizeof(%s), 0, NULL, &event)",
              rhsPropId2->getIdentifier(), id->getIdentifier(), copySize,
              convertToCppType(type->getInnerTargetType()));
      targetFile.pushString(strBuffer);
      targetFile.pushstr_newL(";");
    } else
...
void dsl_cpp_generator::generateStatement(statement *stmt, int isMainFile) {
  ...
  if (stmt->getTypeofNode() == NODE_PROCCALLSTMT) {
    ...
    //generate_exprProcCall(((proc_callStmt*) stmt)->getProcCallExpr(),
    //isMainFile);
  }
...
void dsl_cpp_generator::generateBlock(blockStatement *blockStmt,
                                      bool includeBrace, int isMainFile) {
  // cout << "i am inside generateBlock for the first time and the value of
  // isMainFile=" << isMainFile<<endl;
  dslCodePad &targetFile = getTargetFile(isMainFile);
  ...
  for (Identifier *iden : vars) {
    sprintf(strBuffer, "status = clReleaseMemObject(d_%s);",
            iden->getIdentifier());
    targetFile.pushstr_newL(strBuffer);
...
void dsl_cpp_generator::generateCudaMemCpyParams(
    list<formalParam *> paramList) {
  ...
  for (itr = paramList.begin(); itr != paramList.end(); itr++) {
    ...
    if (type->isPropType()) {
      if (type->getInnerTargetType()->isPrimitiveType()) {
        ...
        generateCudaMemcpyStr(hostvar, temp2,
                              convertToCppType(type->getInnerTargetType()),
                              sizeofProp, 0);
        // generateCudaMalloc(type, (*itr)->getIdentifier()->getIdentifier());
...
void dsl_cpp_generator::setCurrentFunc(Function *func) { currentFunc = func; }

Function *dsl_cpp_generator::getCurrentFunc() { return currentFunc; }
...
void dsl_cpp_generator::generation_begin() {
  ...
  // header.pushstr_newL("graph &g = NULL;");  //temporary fix - to fix the
  // PageRank graph g instance
