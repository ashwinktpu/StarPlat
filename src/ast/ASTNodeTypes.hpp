...
public:
  ...
  void setAssign(assignment *assign) { assignExpr = assign; }
  void setExpression(Expression *expr) { expression = expr; }
  void setAssignFlag() { assign = true; }
  void setExpressionFlag() { expressionflag = true; }

  assignment *getAssignExpr() { return assignExpr; }
  Expression *getExpr() { return expression; }

  bool isAssignExpr() { return assign; }

  bool isExpr() { return expressionflag; }
};
class tempNode {
public:
...
private:
  ...
  char *identifier;
  Expression
      *assignedExpr; /*added to store node/edge property initialized values.
  ...
  bool redecl;       /* added for fixedPoint.(node property parameter)
                      ...
                      - it checks if double buffering is required for a node/edge
                      prop.*/
  ...
  Identifier *updates_association; /* for update.source/destination, get the
                                      updates to which this update belongs to.*/
...
public:
  static Identifier *createIdNode(const char *id) {
    ...
    // std::cout<<"IDENTIFIER = "<<idNode->getIdentifier()<<"
    // "<<strlen(idNode->getIdentifier());
    return idNode;
  }
  ...
  void set_redecl() { redecl = true; }

  bool require_redecl() { return redecl; }
  ...
  void set_forall_filter_association() { forall_filter_association = true; }

  bool get_forall_filter_association() { return forall_filter_association; }

  void set_fpassociation() { fp_association = true; }

  bool get_fp_association() { return fp_association; }

  void setLocalMapReq() { localMap = true; }

  bool isLocalMapReq() { return localMap; }

  void set_fpId(char *fp_sentId) { fpId = fp_sentId; }

  char *get_fpId() { return fpId; }

  void set_fpIdNode(Identifier *fp_sentIdNode) { fpIdNode = fp_sentIdNode; }

  Identifier *get_fpIdNode() { return fpIdNode; }

  void set_assignedExpr(Expression *assignExprSent) {
    assignedExpr = assignExprSent;
  }

  Expression *get_assignedExpr() { return assignedExpr; }

  void set_dependentExpr(Expression *dependExpr) // Expression in fixedPoint of
  ...
  Expression *get_dependentExpr() { return dependentExpr; }

  void setUpdateAssociation(Identifier *updateId_sent) {
    updates_association = updateId_sent;
  }

  Identifier *getUpdateAssociation() { return updates_association; }
};
...
public:
  ...
  static PropAccess *createPropAccessNode(Identifier *id1,
                                          Expression *propExpr) {
    PropAccess *propAccessNode = new PropAccess();
    propAccessNode->identifier1 = id1;
  ...
  Identifier *getIdentifier1() { return identifier1; }
  Identifier *getIdentifier2() {
    printf("getting identifier\n");
    return identifier2;
  }

  Expression *getPropExpr() { return propExpr; }
};
...
public:
  ...
  statement(string statementTypeSent) { statementType = statementTypeSent; }

  string getType() { return statementType; }
};
...
public:
  ...
  void addToFront(statement *stmt) { statements.push_front(stmt); }
  ...
  list<statement *> returnStatements() { return statements; }

  void removeStatement(statement *stmt) { statements.remove(stmt); }
...
private:
  ...
  bool initialLockDecl; /* If omp_locks required in some part of function body,
                           set this to initialize the omp_locks in 1st part of
                           func body*/
  bool hasReturn;
  int funcType;
...
public:
  ...
  static Function *createFunctionNode(Identifier *funcId,
                                      list<formalParam *> paramList) {
  ...
  static Function *createStaticFunctionNode(Identifier *funcId,
                                            list<formalParam *> paramList) {
  ...
  static Function *createDynamicFunctionNode(Identifier *funcId,
                                             list<formalParam *> paramList) {
  ...
  void setFuncType(FUNCTYPE type) { funcType = type; }
  ...
  Identifier *getIdentifier() { return functionId; }
  list<formalParam *> getParamList() { return paramList; }

  blockStatement *getBlockStatement() { return blockstmt; }
...
public:
  ...
  static Type *createForCollectionType(int typeIdSent, int rootTypeSent,
                                       Identifier *TargetGraphSent) {
    Type *type = new Type();
    type->typeId = typeIdSent;
    type->rootType = rootTypeSent;
  ...
  static Type *createForPropertyType(int typeIdSent, int rootTypeSent,
                                     Type *innerTargetTypeSent) {
    Type *type = new Type();
    type->typeId = typeIdSent;
    type->rootType = rootTypeSent;
  ...
  static Type *createForContainerType(int typeIdSent, Type *innerType,
  ...
  Type *copy() {
    ...
    copyType->TargetGraph =
        (this->TargetGraph == NULL) ? NULL : this->TargetGraph->copy();
    copyType->sourceGraph =
        (this->sourceGraph == NULL) ? NULL : this->sourceGraph->copy();
    copyType->innerTargetType =
        (this->innerTargetType == NULL) ? NULL : this->innerTargetType->copy();
    copyType->innerTypeSize =
        (this->innerTypeSize == NULL) ? NULL : this->innerTypeSize->copy();
  ...
  int getRootType() { return rootType; }

  int gettypeId() { return typeId; }
  bool isNodeEdgeType() { return check_isNodeEdgeType(typeId); }

  bool isPropType() { return check_isPropType(typeId); }

  bool isCollectionType() { return check_isCollectionType(typeId); }

  bool isGraphType() { return check_isGraphType(typeId); }
  bool isPrimitiveType() { return check_isPrimitiveType(typeId); }

  bool isPropNodeType() { return check_isPropNodeType(typeId); }

  bool isPropEdgeType() { return check_isPropEdgeType(typeId); }

  bool isListCollection() { return check_isListCollectionType(typeId); }

  bool isSetCollection() { return check_isSetCollectionType(typeId); }
  ...
  Identifier *getTargetGraph() { return TargetGraph; }
  Identifier *getSourceGraph() { return sourceGraph; }
  void setTargetGraph(Identifier *id) { TargetGraph = id; }

  void addToPropList(Identifier *id) {
    graphPropList.push_back(id->getIdentifier());
  }

  list<char *> getPropList() { return graphPropList; }

  Type *getInnerTargetSize() { return innerTypeSize; }

  list<argument *> getArgList() { return sizeExprList; }
};
...
public:
  ...
  static formalParam *createFormalParam(Type *typeSent,
...
public:
  ...
  static Expression *nodeForArithmeticExpr(Expression *left, Expression *right,
  ...
  static Expression *nodeForRelationalExpr(Expression *left, Expression *right,
  ...
  static Expression *nodeForLogicalExpr(Expression *left, Expression *right,
                                        int logicalOperator) {
  ...
  static Expression *nodeForIndexExpr(Expression *expr, Expression *indexExpr,
                                      int operatorType) {
    ...
    cout << "find map expr "
         << indexExpression->mapExpr->getId()->getIdentifier() << "\n";
  ...
  void alterBoolValue() { booleanConstant = !booleanConstant; }
  bool isBooleanLiteral() { return (typeofExpr == EXPR_BOOLCONSTANT); }
  bool isArithmetic() { return (typeofExpr == EXPR_ARITHMETIC); }

  bool isRelational() { return (typeofExpr == EXPR_RELATIONAL); }
  bool isLogical() { return (typeofExpr == EXPR_LOGICAL); }
  bool isUnary() { return (typeofExpr == EXPR_UNARY); }

  bool isIdentifierExpr() { return (typeofExpr == EXPR_ID); }
  bool isPropIdExpr() { return (typeofExpr == EXPR_PROPID); }
  bool isLiteral() {
    return (typeofExpr == EXPR_BOOLCONSTANT || typeofExpr == EXPR_INTCONSTANT ||
            typeofExpr == EXPR_FLOATCONSTANT);
  }
  bool isInfinity() { return (typeofExpr == EXPR_INFINITY); }
  bool isProcCallExpr() { return (typeofExpr == EXPR_PROCCALL); }
  bool isIndexExpr() { return (typeofExpr == EXPR_MAPGET); }

  Expression *getIndexExpr() { return indexExpr; }

  Expression *getMapExpr() { return mapExpr; }

  Expression *getUnaryExpr() { return unaryExpr; }
  ...
  PropAccess *getPropId() { return propId; }

  int getOperatorType() { return operatorType; }

  long getIntegerConstant() { return integerConstant; }

  bool getBooleanConstant() { return booleanConstant; }

  double getFloatConstant() { return floatConstant; }

  bool isPositiveInfinity() { return infinityType; }

  void setTypeofExpr(int type) { overallType = type; }

  int getTypeofExpr() { return overallType; }

  void setExpressionFamily(int exprFamily) { typeofExpr = exprFamily; }

  int getExpressionFamily() { return typeofExpr; }
...
public:
  returnStmt() { returnExpression = NULL; }
  ...
  Expression *getReturnExpression() { return returnExpression; }
};

class batchBlock : public statement {
private:
  blockStatement *statements;
  Identifier *updateId; /* contains the update Id over which this batchblock
                           operates on */
...
public:
  ...
  static batchBlock *createNodeForBatchBlock(Identifier *updatesIdSent,
                                             Expression *batchSizeExprSent,
                                             statement *statements_sent) {
  ...
  blockStatement *getStatements() { return statements; }

  Expression *getBatchSizeExpr() { return batchSizeExpr; }
  ...
  Identifier *getUpdateId() { return updateId; }
};
...
public:
  ...
  static declaration *normal_Declaration(Type *typeSent,
                                         Identifier *identifierSent) {
  ...
  static declaration *assign_Declaration(Type *typeSent,
                                         Identifier *identifierSent,
                                         Expression *expression) {
    declaration *decl = new declaration();
    decl->type = typeSent;
  ...
  Type *getType() { return type; }
  Identifier *getdeclId() { return identifier; }
  Expression *getExpressionAssigned() { return exprAssigned; }

  bool isInitialized() { return (exprAssigned != NULL); }

  void setInGPU(bool inGPU) { this->inGPU = inGPU; }

  bool getInGPU() { return inGPU; }
  void setPropCopy() { propCopy = true; }

  void setMapPropCopy() { mapPropCopy = true; }

  bool isMapPropCopy() { return mapPropCopy; }

  bool isPropCopy() { return propCopy; }
};
...
public:
  ...
  static assignment *id_assignExpr(Identifier *identifierSent,
                                   Expression *expressionSent) {
  ...
  static assignment *prop_assignExpr(PropAccess *propId,
                                     Expression *expressionSent) {
    assignment *assign = new assignment();
  ...
  static assignment *indexAccess_assignExpr(Expression *indexAccess,
                                            Expression *expressionSent) {
  ...
  bool lhs_isIdentifier() { return (lhsType == 1); }

  bool lhs_isProp() { return (lhsType == 2); }

  bool lhs_isIndexAccess() { return (lhsType == 3); }

  Identifier *getId() { return identifier; }

  PropAccess *getPropId() { return propId; }

  Expression *getIndexAccess() { return indexAccess; }

  Expression *getExpr() { return exprAssigned; }
  int getLhsType() { return lhsType; }
  void addAtomicSignal() {
    if (exprAssigned->isArithmetic() || exprAssigned->isLogical())
      atomicSignal = true;
  }

  bool getAtomicSignal() { return atomicSignal; }

  void flagAsDeviceVariable() {
    //~ if(exprAssigned->isArithmetic()||exprAssigned->isLogical())
    deviceVariable = true;
  }
  bool isDeviceAssignment() { return deviceVariable; }
  void flagAccumulateKernel() {
    //~ if(exprAssigned->isArithmetic()||exprAssigned->isLogical())
    accumulateKernel = true;
  }
  bool isAccumulateKernel() { return accumulateKernel; }

  void setPropCopy() { isPropCopy = true; }

  bool hasPropCopy() { return isPropCopy; }
};
...
public:
  ...
  static whileStmt *create_whileStmt(Expression *iterConditionSent,
                                     blockStatement *bodySent) {
  ...
  Expression *getCondition() { return iterCondition; }

  statement *getBody() { return body; }

  void setBody(blockStatement *bodySent) { body = bodySent; }
};
...
public:
  ...
  static dowhileStmt *create_dowhileStmt(Expression *iterConditionSent,
                                         blockStatement *bodySent) {
...
public:
  ...
  static fixedPointStmt *createforfixedPointStmt(Identifier *fixedPointIdSent,
                                                 Expression *dependentPropSent,
                                                 statement *body) {
  ...
  Expression *getDependentProp() { return dependentProp; }

  Identifier *getFixedPointId() { return fixedPointId; }
  statement *getBody() { return body; }
...
public:
  ...
  static ifStmt *create_ifStmt(Expression *condition, statement *ifBodySent,
                               statement *thenBodySent) {
  ...
  statement *getIfBody() { return ifBody; }
  statement *getElseBody() { return thenBody; }
};
...
public:
  ...
  static iterateReverseBFS *
  nodeForRevBFS(Expression *booleanExpr,
                /*Expression* filterExpr,*/ statement *body) {
  ...
  Expression *getBFSFilter() { return booleanExpr; }
...
public:
  ...
  static proc_callExpr *nodeForProc_Call(Identifier *id1, Identifier *id2,
                                         Identifier *methodId,
                                         list<argument *> argList,
  ...
  Identifier *getMethodId() { return methodId; }

  Identifier *getId1() { return id1; }

  Identifier *getId2() { return id2; }

  list<argument *> getArgList() { return argList; }

  void addToArgList(argument *arg) { argList.push_back(arg); }
...
public:
  ...
  static onDeleteBlock *createNodeForOnDeleteBlock(Identifier *iteratorSent,
                                                   Identifier *sourceId,
                                                   proc_callExpr *sourceFunc,
                                                   statement *statements_sent) {
  ...
  Identifier *getIteratorId() { return itertorId; }

  Identifier *getUpdateId() { return updateId; }

  proc_callExpr *getUpdateFunc() { return updateFunc; }
};
...
public:
  ...
  static onAddBlock *createNodeForOnAddBlock(Identifier *iteratorSent,
                                             Identifier *sourceId,
                                             proc_callExpr *sourceFunc,
                                             statement *statements_sent) {
...
public:
  ...
  static iterateBFS *nodeForIterateBFS(Identifier *iterator,
                                       Identifier *graphId,
                                       proc_callExpr *nodeCall,
                                       Identifier *rootNode,
                                       Expression *filterExpr, statement *body,
  ...
  Identifier *getRootNode() { return rootNode; }

  Identifier *getIteratorNode() { return iterator; }

  Identifier *getGraphCandidate() { return graphId; }

  statement *getBody() { return body; }

  iterateReverseBFS *getRBFS() { return revBFS; }

  proc_callExpr *getMethodCall() { return nodeCall; }
...
public:
  ...
  proc_callExpr *getProcCallExpr() { return procCall; }
};
...
private:
  ...
  list<Identifier *>
      doubleBufferVars; // the propnodes which need to be double buffered
  MetaDataUsed metadata;
...
public:
  ...
  static forallStmt *createforallStmt(Identifier *iterator,
                                      Identifier *sourceGraph,
                                      proc_callExpr *extractElemFunc,
                                      statement *body, Expression *filterExpr,
                                      bool isforall) {
  ...
  static forallStmt *createforForStmt(Identifier *iterator, Identifier *source,
                                      statement *body, bool isforall) {
    forallStmt *new_forallStmt = new forallStmt();
    new_forallStmt->iterator = iterator;
  ...
  static forallStmt *id_createforForStmt(Identifier *iterator,
                                         Identifier *source, statement *body,
                                         bool isforall) {
    forallStmt *new_forallStmt = new forallStmt();
    new_forallStmt->iterator = iterator;
    new_forallStmt->source = source;
  ...
  static forallStmt *propId_createforForStmt(Identifier *iterator,
                                             PropAccess *source,
                                             statement *body, bool isforall) {
    forallStmt *new_forallStmt = new forallStmt();
    new_forallStmt->iterator = iterator;
  ...
  static forallStmt *indexExpr_createforForStmt(Identifier *iterator,
                                                Expression *indexExpr,
                                                statement *body,
                                                bool isforall) {
  ...
  bool isForall() { return isforall; }

  /* disable parallelization of for
     for specific instances*/
  void disableForall() { isforall = false; }

  proc_callExpr *getExtractElementFunc() { return extractElemFunc; }

  Expression *getSourceExpr() { return sourceExpr; }

  Identifier *getIterator() { return iterator; }

  PropAccess *getPropSource() { return sourceProp; }

  Identifier *getSource() { return source; }
  ...
  bool isSourceProcCall() { return (extractElemFunc != NULL); }

  bool isSourceExpr() { return (sourceExpr != NULL); }

  bool hasFilterExpr() { return (filterExpr != NULL); }
  void setReductionStatement(statement *stmt) {
    containsreductionStatement = true;
    reductionStatement = stmt;
  }
  bool containsReductionStatement() { return containsreductionStatement; }
  statement *getReductionStatement() { return reductionStatement; }
  void setAssocExpr(Expression *filterExprSent) { assocExpr = filterExprSent; }

  Expression *getAssocExpr() { return assocExpr; }

  void setFilterExprAssoc() { filterExprAssoc = true; }

  bool hasFilterExprAssoc() { return filterExprAssoc; }
  statement *getBody() { return body; }
  void setBody(statement *bodySent) { body = bodySent; }
  Expression *getfilterExpr() { return filterExpr; }
  ...
  set<int> get_reduceKeys() { return reduc_keys; }

  list<Identifier *> get_reduceIds(int key) { return reduction_map[key]; }
  ...
  list<Identifier *> getUsedVariables() { return this->usedVars; }
  ...
  void addDoubleBufferVar(Identifier *var) { doubleBufferVars.push_back(var); }

  list<Identifier *> getDoubleBufferVars() { return doubleBufferVars; }

  MetaDataUsed getMetaDataUsed() { return metadata; }

  void pushMapLocals(Identifier *id) { mapLocals.insert(id); }

  set<Identifier *> getMapLocal() { return mapLocals; }
...
public:
  ...
  static reductionCall *nodeForReductionCall(int reduceType,
                                             list<argument *> argList) {
...
public:
  ...
  static reductionCallStmt *id_reducCallStmt(Identifier *id,
                                             reductionCall *reducCall) {
  ...
  static reductionCallStmt *propId_reducCallStmt(PropAccess *propId,
                                                 reductionCall *reducCall) {
    reductionCallStmt *reducCallStmtNode = new reductionCallStmt();
  ...
  static reductionCallStmt *id_reduc_opStmt(Identifier *id, int reduce_op,
                                            Expression *rightSide) {
    reductionCallStmt *reducCallStmtNode = new reductionCallStmt();
    reducCallStmtNode->id = id;
  ...
  static reductionCallStmt *propId_reduc_opStmt(PropAccess *propId,
                                                int reduce_op,
                                                Expression *rightSide) {
    reductionCallStmt *reducCallStmtNode = new reductionCallStmt();
    reducCallStmtNode->propAccessId = propId;
  ...
  static reductionCallStmt *
  leftList_reducCallStmt(list<ASTNode *> llist, reductionCall *reducCall,
                         list<ASTNode *> exprListSent) {
    // cout<<"REDUC CALL TYPE
    // "<<(reducCall->getReductionType()==REDUCE_MIN)<<"\n";
    reductionCallStmt *reducCallStmtNode = new reductionCallStmt();
  ...
  int getLhsType() { return lhsType; }
  bool isLeftIdentifier() { return (id != NULL); }

  reductionCall *getReducCall() { return reducCall; }

  Identifier *getLeftId() { return id; }

  PropAccess *getPropAccess() { return propAccessId; }
  bool isListInvolved() { return (exprList.size() != 0); }

  list<ASTNode *> getLeftList() { return leftList; }
  list<ASTNode *> getRightList() { return exprList; }

  Expression *getRightSide() { return rightSide; }

  int reduction_op() { return reduc_op; }
