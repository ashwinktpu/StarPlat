...
/*TO be implemented. It will contain functions that will be called by action
 * part of Parser  for building the nodes of AST*/
...
public:
  ...
  static void resetTemp(vector<Identifier *> &tempIds) {
    ...
    for (Identifier *id : tempIds) {
      printf("tempID %s funccount %d\n", id->getIdentifier(),
             frontEndContext.getCurrentFuncCount());
      graphId[currentFuncType][frontEndContext.getCurrentFuncCount()].push_back(
          id);
    }
  }
  ...
  static ASTNode *createFuncNode(ASTNode *id,
                                 list<formalParam *> formalParamList) {
    Identifier *funcId = (Identifier *)id;
    // Type* retType = (Type*)retType;

    Function *functionNode =
        Function::createFunctionNode(funcId, formalParamList /**, retType*/);
    return functionNode;
  }

  static ASTNode *createStaticFuncNode(
      ASTNode *id, list<formalParam *> formalParamList /*, ASTNode* retType*/) {
    Identifier *staticFuncId = (Identifier *)id;
    // Type* retType = (Type*)retType;

    Function *staticFuncNode = Function::createStaticFunctionNode(
        staticFuncId, formalParamList /*, retType*/);
    return staticFuncNode;
  }

  static ASTNode *createDynamicFuncNode(ASTNode *id,
                                        list<formalParam *> formalParamList) {
    Identifier *dynFuncId = (Identifier *)id;
    // Type* retType = (Type*)retType;

    Function *dynFuncNode = Function::createDynamicFunctionNode(
        dynFuncId, formalParamList /*, retType*/);
    return dynFuncNode;
  }
  ...
  static ASTNode *createIncrementalNode(list<formalParam *> formalParamList) {
    ...
    Function *incrementalNode =
        Function::createIncrementalNode(formalParamList /*, retType*/);

    return incrementalNode;
  }
  static ASTNode *createDecrementalNode(list<formalParam *> formalParamList) {
    // Type* retType = (Type*)retType;
    Function *decrementalNode =
        Function::createDecrementalNode(formalParamList /*, retType*/);
  ...
  static void addToBlock(
      ASTNode
          *statementNode) { // cout<<"Inside ADD BLOCK"<<statementNode<<"\n";
  ...
  static ASTNode *createParamNode(ASTNode *type, ASTNode *id) {
    ...
    formalParam *formalParamNode =
        formalParam::createFormalParam((Type *)type, (Identifier *)id);
    return formalParamNode;
  }

  static ASTNode *createNormalDeclNode(ASTNode *type, ASTNode *id) {
    declaration *declNode =
        declaration::normal_Declaration((Type *)type, (Identifier *)id);
    return declNode;
  }

  static ASTNode *createAssignedDeclNode(ASTNode *type, ASTNode *id,
                                         ASTNode *exprAssigned) {
    declaration *declNode = declaration::assign_Declaration(
        (Type *)type, (Identifier *)id, (Expression *)exprAssigned);
    return declNode;
  }
  static ASTNode *createPrimitiveTypeNode(int typeId) { // cout<<"Inside Func";
  ...
  static ASTNode *createCollectionTypeNode(int typeId, ASTNode *targetGraph) {
    Type *typeNode =
        Type::createForCollectionType(typeId, 3, (Identifier *)targetGraph);
    return typeNode;
  }
  static ASTNode *createPropertyTypeNode(int typeId, ASTNode *innerTargetType) {
    Type *typeNode =
        Type::createForPropertyType(typeId, 4, (Type *)innerTargetType);
    return typeNode;
  }
  ...
  static ASTNode *createAssignmentNode(ASTNode *leftSide, ASTNode *rhs) {
    ...
    if (leftSide->getTypeofNode() == NODE_ID) {
      assignmentNode =
          assignment::id_assignExpr((Identifier *)leftSide, (Expression *)rhs);
    }
    if (leftSide->getTypeofNode() == NODE_PROPACCESS) {
      assignmentNode = assignment::prop_assignExpr((PropAccess *)leftSide,
                                                   (Expression *)rhs);
    }
    if (leftSide->getTypeofNode() == NODE_EXPR) {
      ...
      if (expr->getExpressionFamily() == EXPR_MAPGET) {
        ...
        cout << "INSIDE MAPGET EXPR"
             << "\n";
        assignmentNode = assignment::indexAccess_assignExpr(
            (Expression *)leftSide, (Expression *)rhs);
      }
    }
  ...
  static ASTNode *createNodeForUnaryStatements(ASTNode *unaryExpr) {
    ...
    assert(leftSideExpr->getExpressionFamily() == EXPR_ID ||
           leftSideExpr->getExpressionFamily() == EXPR_PROPID);
  ...
  static ASTNode *createNodeForProcCall(ASTNode *proc_callId,
    ...
    if (proc_callId->getTypeofNode() == NODE_ID) {
      ...
      if (indexExprSent != NULL) {
        ...
        proc_callExprNode = proc_callExpr::nodeForProc_Call(
            NULL, NULL, (Identifier *)proc_callId, argList, NULL);
      }
    }
    if (proc_callId->getTypeofNode() == NODE_PROPACCESS) {
      ...
      if (indexExprSent != NULL) {
        ...
        Expression *indexExpr = (Expression *)indexExprSent;
        proc_callExprNode = proc_callExpr::nodeForProc_Call(
            propAccessId->getIdentifier1(), NULL,
            propAccessId->getIdentifier2(), argList, indexExpr);

      } else {
        proc_callExprNode = proc_callExpr::nodeForProc_Call(
            propAccessId->getIdentifier1(), NULL,
            propAccessId->getIdentifier2(), argList, NULL);
      }
    }
  ...
  static ASTNode *createContainerTypeNode(int typeId, ASTNode *innerType,
                                          list<argument *> argList,
                                          ASTNode *innerTypeSize) {

    Type *containerNode = Type::createForContainerType(
        typeId, (Type *)innerType, argList, (Type *)innerTypeSize);

    return containerNode;
  }

  static ASTNode* createDSUTypeNode(int typeId) {
    Type *typeNode = Type::createForDSU(typeId, 1);
    return typeNode;
  }
  ...
  static ASTNode *createNodeForArithmeticExpr(ASTNode *expr1, ASTNode *expr2,
                                              int operatorType) {
    Expression *arithmeticExprNode = Expression::nodeForArithmeticExpr(
        (Expression *)expr1, (Expression *)expr2, operatorType);
    return arithmeticExprNode;
  }
  static ASTNode *createNodeForRelationalExpr(ASTNode *expr1, ASTNode *expr2,
                                              int operatorType) {
    Expression *relationalExprNode = Expression::nodeForRelationalExpr(
        (Expression *)expr1, (Expression *)expr2, operatorType);
    return relationalExprNode;
  }
  static ASTNode *createNodeForLogicalExpr(ASTNode *expr1, ASTNode *expr2,
                                           int operatorType) {
    Expression *logicalExprNode = Expression::nodeForLogicalExpr(
        (Expression *)expr1, (Expression *)expr2, operatorType);
  ...
  static ASTNode *createNodeForUnaryExpr(ASTNode *expr, int operatorType) {
    Expression *unaryExpr =
        Expression::nodeForUnaryExpr((Expression *)expr, operatorType);
    return unaryExpr;
  }

  static ASTNode *createNodeForIndexExpr(ASTNode *expr, ASTNode *indexExpr,
                                         int operatorType) {
    Expression *indexExpression = Expression::nodeForIndexExpr(
        (Expression *)expr, (Expression *)indexExpr, operatorType);
  ...
  static ASTNode *createReturnStatementNode(ASTNode *returnExpression) {
    ...
    returnStmtNode =
        returnStmt::createNodeForReturnStmt((Expression *)returnExpression);

    return returnStmtNode;
  }

  static ASTNode *createBatchBlockStmt(ASTNode *updatesId,
                                       ASTNode *batchSizeExpr,
                                       ASTNode *blockStmts) {
    statement *batchBlockStmtNode;
    batchBlockStmtNode = batchBlock::createNodeForBatchBlock(
        (Identifier *)updatesId, (Expression *)batchSizeExpr,
        (statement *)blockStmts);
    return batchBlockStmtNode;
  }

  static ASTNode *createOnAddBlock(ASTNode *updateIterator,
                                   ASTNode *updateSource, ASTNode *updateFunc,
                                   ASTNode *blockStmts) {
    statement *onAddBlockNode;
    onAddBlockNode = onAddBlock::createNodeForOnAddBlock(
        (Identifier *)updateIterator, (Identifier *)updateSource,
        (proc_callExpr *)updateFunc, (statement *)blockStmts);

    return onAddBlockNode;
  }

  static ASTNode *createOnDeleteBlock(ASTNode *updateIterator,
                                      ASTNode *updateSource,
                                      ASTNode *updateFunc,
                                      ASTNode *blockStmts) {
    statement *onDeleteBlockNode;
    onDeleteBlockNode = onDeleteBlock::createNodeForOnDeleteBlock(
        (Identifier *)updateIterator, (Identifier *)updateSource,
        (proc_callExpr *)updateFunc, (statement *)blockStmts);
  ...
  static ASTNode *createNodeForFixedPointStmt(ASTNode *fixedPointId,
                                              ASTNode *dependentProp,
                                              ASTNode *body) {
    statement *fixedPointStmtNode;
    fixedPointStmtNode = fixedPointStmt::createforfixedPointStmt(
        (Identifier *)fixedPointId, (Expression *)dependentProp,
        (blockStatement *)body);
    return fixedPointStmtNode;
  }
  static ASTNode *createNodeForWhileStmt(ASTNode *iterCondition,
                                         ASTNode *body) {
    statement *whileStmtNode;
    whileStmtNode = whileStmt::create_whileStmt((Expression *)iterCondition,
                                                (blockStatement *)body);
    return whileStmtNode;
  }
  static ASTNode *createNodeForDoWhileStmt(ASTNode *iterCondition,
                                           ASTNode *body) {
    statement *dowhileStmtNode;
    dowhileStmtNode = dowhileStmt::create_dowhileStmt(
        (Expression *)iterCondition, (blockStatement *)body);
    return dowhileStmtNode;
  }
  static ASTNode *createNodeForIfStmt(ASTNode *iterCondition, ASTNode *thenBody,
                                      ASTNode *elseBody) {
    statement *ifStmtNode;
    ifStmtNode =
        ifStmt::create_ifStmt((Expression *)iterCondition,
                              (statement *)thenBody, (statement *)elseBody);
    return ifStmtNode;
  }
  static ASTNode *createNodeForForAllStmt(ASTNode *iterator,
                                          ASTNode *sourceGraph,
                                          ASTNode *extractElemFunc,
                                          ASTNode *filterExpr, ASTNode *body,
                                          bool isforall) {
    ...
    forallStmtNode = forallStmt::createforallStmt(
        id, id1, (proc_callExpr *)extractElemFunc, (statement *)body,
        (Expression *)filterExpr, isforall);
    return forallStmtNode;
  }
  static ASTNode *createNodeForForStmt(ASTNode *iterator, ASTNode *source,
                                       ASTNode *body, bool isforall) {
    statement *forallStmtNode;
    Identifier *id = (Identifier *)iterator;

    if (source->getTypeofNode() == NODE_ID)
      forallStmtNode = forallStmt::id_createforForStmt(
          id, (Identifier *)source, (statement *)body, isforall);

    if (source->getTypeofNode() == NODE_PROPACCESS)
      forallStmtNode = forallStmt::propId_createforForStmt(
          id, (PropAccess *)source, (statement *)body, isforall);

    if (source->getTypeofNode() == NODE_EXPR) {
      Expression *expr = (Expression *)source;
      if (expr->getExpressionFamily() == EXPR_MAPGET) {

        forallStmtNode = forallStmt::indexExpr_createforForStmt(
            id, (Expression *)expr, (statement *)body, isforall);
      }
    }

    return forallStmtNode;
  }
  static ASTNode *createNodeforReductionCall(int reductionOperationType,
                                             list<argument *> argList) {
    reductionCall *reductionCallNode =
        reductionCall::nodeForReductionCall(reductionOperationType, argList);
    return reductionCallNode;
  }
  static ASTNode *createNodeForReductionStmt(ASTNode *leftSide,
                                             ASTNode *reductionCallNode) {
    reductionCallStmt *reductionStmtNode;
    if (leftSide->getTypeofNode() == NODE_ID) {
      reductionStmtNode = reductionCallStmt::id_reducCallStmt(
          (Identifier *)leftSide, (reductionCall *)reductionStmtNode);
    }
    if (leftSide->getTypeofNode() == NODE_PROPACCESS) {
      reductionStmtNode = reductionCallStmt::propId_reducCallStmt(
          (PropAccess *)leftSide, (reductionCall *)reductionStmtNode);
    }
    return reductionStmtNode;
  }
  static ASTNode *createNodeForReductionStmtList(list<ASTNode *> leftList,
                                                 ASTNode *reductionCallNode,
                                                 list<ASTNode *> exprList) {
    reductionCallStmt *reductionStmtNode;
    reductionStmtNode = reductionCallStmt::leftList_reducCallStmt(
        leftList, (reductionCall *)reductionCallNode, exprList);
    return reductionStmtNode;
  }

  static ASTNode *createNodeForReductionOpStmt(ASTNode *leftSide,
                                               int reduction_op,
                                               ASTNode *rightSide) {
    reductionCallStmt *reductionStmtNode;
    if (leftSide->getTypeofNode() == NODE_ID) {
      reductionStmtNode = reductionCallStmt::id_reduc_opStmt(
          (Identifier *)leftSide, reduction_op, (Expression *)rightSide);
    }
    if (leftSide->getTypeofNode() == NODE_PROPACCESS) {
      reductionStmtNode = reductionCallStmt::propId_reduc_opStmt(
          (PropAccess *)leftSide, reduction_op, (Expression *)rightSide);
    }
  ...
  static ASTNode *createPropIdNode(ASTNode *id1, ASTNode *id2) {
    ...
    if (id2->getTypeofNode() == NODE_ID)
      propIdNode = PropAccess::createPropAccessNode((Identifier *)id1,
                                                    (Identifier *)id2);
  ...
  static ASTNode *
  createIterateInReverseBFSNode(ASTNode *booleanExpr,
                                /*ASTNode* filterExpr,*/ ASTNode *body) {
    iterateReverseBFS *iterateReverseBFSNode;
    iterateReverseBFSNode = iterateReverseBFS::nodeForRevBFS(
        (Expression *)booleanExpr,
        /*(Expression*)filterExpr,*/ (statement *)body);
    return iterateReverseBFSNode;
  }
  static ASTNode *createIterateInBFSNode(ASTNode *iterator, ASTNode *graphId,
                                         ASTNode *procCall, ASTNode *rootNode,
                                         ASTNode *filterExpr, ASTNode *body,
                                         ASTNode *revBFS) {
    ...
    if (revBFS != NULL)
      iterateBFSNode = iterateBFS::nodeForIterateBFS(
          id1, id2, nodeCall, id3, (Expression *)filterExpr, (statement *)body,
          (iterateReverseBFS *)revBFS);
    else
      iterateBFSNode = iterateBFS::nodeForIterateBFS(id1, id2, nodeCall, id3,
