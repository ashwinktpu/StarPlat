...
enum codeStructure {INVALID_STUCTURE, MIN_PROP_UPDATE, PROP_UPDATE};
                ...
                for(statement* stmt: ifBody->returnStatements())
                {
                    if(stmt->getTypeofNode() == NODE_ASSIGN)
                    {
                        ...
                        if(assgn->lhs_isProp())
                        {
                            ...
                            if(checkIdEqual(ifPropId->getIdentifier1(), propId->getIdentifier1()) 
                                && checkIdEqual(ifPropId->getIdentifier2(), propId->getIdentifier2()))
                            {
                ...
                if (checkIdEqual(srcGraph, srcGraph1) && (strcmp(procCall1->getMethodId()->getIdentifier(), "neighbors") == 0))
                {
                        ...
                        checkIdEqual(argVal->getId(), itr))
                    {
                        ...
                        for(statement* istmt: forBody->returnStatements())
                        {
                            ...
                            {
                                reductionCallStmt* modifiedStmt = 
                            }
                        }
                    }
                }
