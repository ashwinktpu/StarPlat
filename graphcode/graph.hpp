...
public:
  ...
  // library function to check candidate vertex is in the path from root to dest
  // in SPT.
  ...
  void addEdge(int src, int dest) {
    ...
    if (edgeList[startIndex] >= dest || nbrsCount == 0)
      ...
      for (int i = startIndex; i < endIndex - 1;
           i++) // find the correct index to insert.
      {
    ...
    edgeLen[insertAt] =
        1; // to be changed. the weight should be from paramters.
...
#pragma omp parallel for
  ...
  std::vector<update> getDeletesFromBatch(int updateIndex, int batchSize,
                                          std::vector<update> updateVec) {
    std::vector<update> deleteVec =
        getDeletions(updateIndex, batchSize, updateVec);
    return deleteVec;
  }

  std::vector<update> getAddsFromBatch(int updateIndex, int batchSize,
                                       std::vector<update> updateVec) {
    std::vector<update> addVec =
        getAdditions(updateIndex, batchSize, updateVec);
    return addVec;
  }
  ...
  /**
   * Parses the graph and creates the CSR representation.
   * TODO: Is it necessary to create the CSR here?
   */
  void parseGraph() {
    parseEdges();
    printf("finished parsing edges\n");
    printf("creating csr arrays\n");

#pragma omp parallel for

    // sort out-neighbours of each edge by value
    ...
    */
    for (int i = 0; i <= nodesTotal; i++) // change to 1-nodesTotal.
    {
      ...
      // TODO: might be replaceable by a memset
      perNodeCSRSpace[i] = 0;
...
#pragma omp parallel for num_threads(4)
    for (int i = 0; i <= nodesTotal; i++) {
      for (int j = indexofNodes[i]; j < indexofNodes[i + 1]; j++) {
        ...
        vertexInter[index_in_srcList] =
            srcList[index_in_srcList]; /*store the original content of srcList
...
#pragma omp parallel for num_threads(4)
    for (int i = 0; i <= nodesTotal; i++) {
      std::vector<int> vect;
      vect.insert(vect.begin(), srcList + rev_indexofNodes[i],
                  srcList + rev_indexofNodes[i + 1]);
...
#pragma omp parallel for
    ...
    updateCSRDel_omp(num_nodes, edgesTotal, indexofNodes, edgeList,
                     rev_indexofNodes, srcList, edgeLen, diff_edgeLen,
                     diff_indexofNodes, diff_edgeList, diff_rev_indexofNodes,
                     diff_rev_edgeList, rev_edgeLen, diff_rev_edgeLen,
                     perNodeUpdateInfo, perNodeUpdateRevInfo, perNodeCSRSpace,
                     perNodeRevCSRSpace, slicedUpdates);
  ...
  void updateCSRAdd(std::vector<update> &batchUpdate, int k, int size) {
    ...
    updateCSRAdd_omp(num_nodes, edgesTotal, indexofNodes, edgeList,
                     rev_indexofNodes, srcList, edgeLen, &diff_edgeLen,
                     &diff_indexofNodes, &diff_edgeList, &diff_rev_indexofNodes,
                     &diff_rev_edgeList, rev_edgeLen, &diff_rev_edgeLen,
                     perNodeUpdateInfo, perNodeUpdateRevInfo, perNodeCSRSpace,
  ...
  std::vector<edge> getNeighbors(int node) {
    ...
    if (diff_edgeList != NULL) {
      for (int j = diff_indexofNodes[node]; j < diff_indexofNodes[node + 1];
           j++) {
  ...
  std::vector<edge> getInNeighbors(int node) {
    ...
    if (diff_rev_edgeList != NULL) {
      for (int j = diff_rev_indexofNodes[node];
           j < diff_rev_indexofNodes[node + 1]; j++) {
