...
class Rma_Datatype {
  ...
  void get_lock(int32_t proc_num, locktype lock,  bool no_checks_needed = false)  
  {
    ...
    {   
        ...
        MPI_Win_lock(MPI_LOCK_SHARED, proc_num, MPI_MODE_NOCHECK , win);
    }
  ...
  const char * get_model_type()
  {
    ...
    char* value;
    int valuelenx ;
    const char * key = "MPI_WIN_MODEL";
    MPI_Win_get_attr(win,MPI_WIN_MODEL, &buff, &flag);
    printf("%d\n",*buff);
    if(!flag)
      return "notfound";
    if(*buff == MPI_WIN_SEPARATE)
      printf("noo\n");
    else if(*buff == MPI_WIN_UNIFIED)
      printf("yess\n");
    return "found";  
