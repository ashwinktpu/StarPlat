...
timeout 2h /lfs/sware/openmpi411/bin/mpirun -np 32 galloisDSL.out $graphPath/pokecwt.txt outputActual.txt >> BigTimingsFile

for graph in $graphPath/*
do
	echo $graph
	echo $graph >> BigTimingsFile
	timeout 2h /lfs/sware/openmpi411/bin/mpirun -np 32 galloisDSL.out $graph outputActual.txt >> BigTimingsFile
done
