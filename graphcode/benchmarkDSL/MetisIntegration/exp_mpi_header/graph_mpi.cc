#include "graph_mpi.h"
#include "metisReader/debug.hpp"
#include "metisReader/partitioned_file_reader.hpp"
#include "metisReader/perform_updates.hpp"
#include "metisReader/csrConverter.h"
#include "metisReader/synchronize_p2p.hpp"
#include "metisReader/serial_methods.hpp"
#include <iostream>
#include <fstream>
#include <sstream>
#include <unordered_map>

extern std::unordered_map<int,int> global_to_local ;
extern std::unordered_map<int,int> local_to_global ;
...
void freeAllUpdateMemory (std::vector<int*> updates) {
  ...
  {
    // Barenya :TODO : Support for this function not added yet with METIS Integration. 
    get_lock_for_reduction_statement();
		printf ("dest List is of size %d\n", this->local_edges) ;
		for (int i=0; i<this->local_edges; i++) {
			std::cout <<  this->destList.data[i]  << " " ;
		}
		std::cout << std::endl ;

		for (auto &it : this->local_to_global) {
			std::cout << it.first << " " << it.second << endl ;	
		}
    // for(int i=start_node();i<=end_node();i++)
		for (int i=0;i<=this->local_vertices;i++)
    {
        std::cout<<"node "<<this->local_to_global[i]<<std::endl;
  ...
  Graph::Graph(char* file_name, boost::mpi::communicator world , int32_t undirected, bool optimized)
  {
    ...
    char* local_graph_path = construct_file_name (world.rank()) ;
    FILE* graph_file = fileIO (local_graph_path) ;  // deduce the file name .
    input (graph_file) ; // Generate residual graph. 

    // Letting the undirected graph be formed. That can be used to form reverse distributed csr.

    // Following lines of code to determine global basic information.
    this->local_vertices = v_id;
    this->local_edges = e ;
    this->global_vertices ;
    this->global_edges ;
    ...
    // Mapping local to global and global to local.
		copyBackMaps (this->global_to_local, this->local_to_global) ;
		printf ("size of local to global = %d\n", this->local_to_global.size ()) ;
		printf ("size of global to local = %d\n", this->global_to_local.size ()) ;

    // May remove this barrier. Allreduce causes a stronger barrier of both memory and control.
    world.barrier();

    // csr Conversion class object.
    csrConverter csr (residual_graph, this->local_to_global, this->local_vertices, this->local_edges, world.rank () ) ;

    // Make the csr s with proper Node Properties.
    csr.convert () ;
    int32_t * indexofNodes = csr.getNodeOffsets () ;
    int32_t * destList = csr.getCSRArr () ;
    int32_t * destProc = csr.getDestProc () ;
    int32_t * srcProc = csr.getSrcProc () ;
    int32_t * weights = csr.getEdgeWeights () ;
    int32_t * revMap = csr.getRevMap () ;
    
    nodesPartitionSize = local_vertices ;




    // Form csr portions for reverse graph.
    int32_t * rev_indexofNodes = csr.getRevIdxNodes () ;
    int32_t * srcList = csr.getSrcList () ;

    // Gather information of all the destination lists.
    int32_t destListSize = this->local_edges ;
    ...
    // Create all MPI Windows for OSC.
    this->indexofNodes.mpi_datatype = MPI_INT32_T;
    this->indexofNodes.create_window(indexofNodes, this->local_vertices +1, sizeof(int32_t),world);
    
    this->rev_indexofNodes.mpi_datatype = MPI_INT32_T;
    this->rev_indexofNodes.create_window(rev_indexofNodes, this->local_vertices+1, sizeof(int32_t), world);
    
    this->destList.mpi_datatype = MPI_INT32_T;
    this->destList.create_window(destList, this->local_edges, sizeof(int32_t), world);

    this->srcList.mpi_datatype = MPI_INT32_T;
    this->srcList.create_window(srcList, this->local_edges, sizeof(int32_t), world);

    this->destProc.mpi_datatype = MPI_INT32_T ; 
    this->destProc.create_window(destProc, this->local_edges, sizeof(int32_t), world);


    this->srcProc.mpi_datatype = MPI_INT32_T ;
    this->srcProc.create_window(srcProc, this->local_vertices, sizeof(int32_t), world);
    ...
    */
    ...
    // This needs changing then.

		print_csr () ;

    speedUpForGetEdge = std::vector<std::unordered_map<int, int> > (this->local_vertices) ;
    if (optimized) {
      for (int u = 0; u < this->local_vertices+1; u++) {
        int u_temp = u ;
  ...
  {
    return this->local_to_global[0];
  }

  int Graph::end_node()
  {
    return this->local_to_global[this->local_vertices-1];
  }
  ...
  {
    // Make sure that node is the global id.
    int ownerProcessOfNode = -1 ;
    if (native_vertex.find (node) != native_vertex.end ()) {
      ownerProcessOfNode = world.rank () ;
    } else {
      assert (false) ;
    }
    assert (ownerProcessOfNode >= 0) ;
    return ownerProcessOfNode ;
  }

  int  Graph::get_node_local_index(int node)
  {
    return global_to_local [node];
  }


  int Graph::get_edge_owner(Edge edge)
  {
      // Likely won't be needing this for. Therefore, 
      assert (false ) ;
      int owner = 0;
      if(edge.is_in_csr())
      {
  ...
  {
    // Likely won't be needing this either.
    assert (false ) ;
    if(edge.is_in_csr())
  ...
  Edge Graph::get_edge_i (int u, int v) {
    int owner_proc = get_node_owner (u) ;
    int index = get_node_local_index (u) ;
  ...
  int Graph::get_other_vertex(int u, int v_idx) {
    int owner_proc = get_node_owner (u) ;
    int index = get_node_local_index (v_idx) ;
    if (world.rank () == owner_proc) {
  ...
  void Graph::propagateNodeFlags(NodeProperty<bool>& property)
  {
      ...
      // Barenya : TODO : add support for this function with METIS integration.
      assert (false) ;
