...
#define BACKWARD 1 
	...
	FILE* fileIO(char* file_path){
			...
			char* global = "/lfs/usrhome/mtech/cs22m028/benchmarkDSL/MetisIntegration/header_test_exp/" ;
			int n = strlen (global) ;
...
void copyBackMaps (std::unordered_map<int,int> &globalToLocal, std::unordered_map<int,int> &localToGlobal) {
	for (auto &it:local_to_global) {
		localToGlobal[it.first]=it.second ;	
	}
	for (auto &it:global_to_local) {
		globalToLocal[it.first]=it.second ;
	}
}
