...
std::vector<int> partitionGraph(std::vector<int>& xadj, std::vector<int>& adjncy, std::vector<int>& weights, int numPartitions) {
    ...
    // options[METIS_OPTION_OBJTYPE]=METIS_OBJTYPE_VOL;
		options[METIS_OPTION_NCUTS] = 10; // Number of different partitionings to compute
		options[METIS_OPTION_NITER] = 100; // Maximum number of refinement iterations
		options[METIS_OPTION_CTYPE] = METIS_CTYPE_SHEM; // Use the sorted heavy-edge matching scheme
		options[METIS_OPTION_IPTYPE] = METIS_IPTYPE_GROW; // Use the grow-initial partitioning scheme
