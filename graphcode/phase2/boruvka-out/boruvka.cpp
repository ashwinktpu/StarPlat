...
void Compute_SSSP(graph& g,int * dist,int src)
{
  ...
  status = clSetKernelArg(Compute_SSSP, 7, sizeof(cl_mem), (void *)&d_dist);
  ...
  //Indexdist v initialization
  int initdistvalv = 0;
  status = clSetKernelArg(initIndexdist_kernel , 0,sizeof(cl_mem) ,(void *)&d_dist);
  status = clSetKernelArg(initIndexdist_kernel, 1,sizeof(int), (void*)&v);
  status = clSetKernelArg(initIndexdist_kernel, 2, sizeof(int),(void *)&initdistvalv);
