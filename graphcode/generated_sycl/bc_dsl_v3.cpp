...
void Compute_BC(graph& g,double* BC,std::set<int>& sourceSet)
{
  ...
  double* d_BC;
  d_BC=malloc_device<double>(V, Q);


  //BEGIN DSL PARSING 
  Q.submit([&](handler &h){ h.parallel_for(NUM_THREADS, [=](id<1> i){
    for (; i < V; i += stride) d_BC[i] = (double)0;});
}).wait();
...
double* d_delta;
d_delta=malloc_device<double>(V, Q);
...
for(itr=sourceSet.begin();itr!=sourceSet.end();itr++) 
{
  int src = *itr;
  Q.submit([&](handler &h){ h.parallel_for(NUM_THREADS, [=](id<1> i){
    for (; i < V; i += stride) d_delta[i] = (double)0;});
}).wait();
...
Q.submit([&](handler &h)
{ h.memcpy(      BC,     d_BC, sizeof(double)*(V)); })
.wait();
} //end FUN
