  ...
  std::vector<int>& rnk, boost::mpi::communicator world )
{
  ...
  while (vertex < n ){
    ...
    while (deg > 0 ){
      ...
      {
        int dst = edges.getValue (index);
        records.getValue (vertex).push_back (dst);
...
vertex = 0;
while (vertex < n ){
  std::vector<int> t = par.getValue (vertex);
  records.getValue (vertex).push_back(t);
...
{
  if (g.getOutDegree(v, world) >= degree.getValue (d) )
  {
    bool flag = true;
    int index = 0;
