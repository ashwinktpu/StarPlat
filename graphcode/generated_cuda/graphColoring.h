...
__global__ void colorGraph_kernel_1(int V, int E, int* d_meta, int* d_data, int* d_src, int* d_weight, int *d_rev_meta,bool *d_modified_next,bool* d_modified,long* d_color,bool* d_modified_next){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  ...
  if (d_modified[v] == false){ // if filter begin 
    ...
    {int par = d_src[edge] ;
      total = total + 1;
      if (par == v){ // if filter begin 
        cnt = cnt + 1;

      } // if filter end
      else
      if (d_modified[par] == true){ // if filter begin 
        cnt = cnt + 1;

      } // if filter end
      else
      if (d_color[v] > d_color[par]){ // if filter begin 
        cnt = cnt + 1;
