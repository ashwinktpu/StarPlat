...

{
  ...
  for(itr=sourceSet.begin();itr!=sourceSet.end();itr++)
  {
    ...
    do {
      ...
      //Kernel LAUNCH
      fwd_pass<<<num_blocks, block_size>>>(V, d_meta, d_data,d_weight, d_delta, d_sigma, d_level, d_hops_from_source, d_finished, d_BC);
    ...
    while(hops_from_source > 1) {
      ...
      //KERNEL Launch
      back_pass<<<num_blocks, block_size>>>(V, d_meta, d_data, d_weight, d_delta, d_sigma, d_level, d_hops_from_source, d_finished, d_BC);
...
int main(int argc , char ** argv)
{
  ...
  srcfile.close();
    ...
    double BC = (double *)malloc((g.num_nodes())*sizeof(double))
    Compute_BC(G,BC,src,printAns);
