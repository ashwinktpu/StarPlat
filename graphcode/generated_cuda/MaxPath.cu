...

{
  ...
  double*  s1; // asst in .cu

  long*  s1; // asst in .cu

  std::set<int>& l; // asst in .cu

  cudaMemcpyToSymbol(::x, &x, sizeof(int), 0, cudaMemcpyHostToDevice);
  computeMaxPath_kernel<<<numBlocks, threadsPerBlock>>>(V,E,d_meta,d_data,d_src,d_weight,d_rev_meta,d_modified_next);
