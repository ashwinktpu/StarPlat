...
__global__ void APFB_kernel_6(int V, int E, int* d_meta, int* d_data, int* d_src, int* d_weight, int *d_rev_meta,bool *d_modified_next,int* d_cmatch,int* d_rmatch){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned r = blockIdx.x * blockDim.x + threadIdx.x;
  if(r >= V) return;
