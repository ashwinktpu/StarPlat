...
__global__ void vHong_kernel(int V, int E, int* d_meta, int* d_data, int* d_src, int* d_weight, int *d_rev_meta,bool *d_modified_next,int* d_outDeg,int* d_inDeg){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
...
__global__ void vHong_kernel(int V, int E, int* d_meta, int* d_data, int* d_src, int* d_weight, int *d_rev_meta,bool *d_modified_next,int* d_range,int* d_scc,bool* d_isPivot){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
...
__global__ void vHong_kernel(int V, int E, int* d_meta, int* d_data, int* d_src, int* d_weight, int *d_rev_meta,bool *d_modified_next,int* d_scc,int* d_outDeg,int* d_inDeg,int* d_pivotField,int* d_range){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  if (d_scc[src] == -1){ // if filter begin 
...
__global__ void vHong_kernel(int V, int E, int* d_meta, int* d_data, int* d_src, int* d_weight, int *d_rev_meta,bool *d_modified_next,int* d_scc,int* d_pivotField,int* d_range,bool* d_isPivot,bool* d_visitBw,bool* d_visitFw){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  if (d_scc[src] == -1){ // if filter begin 
...
__global__ void vHong_kernel(int V, int E, int* d_meta, int* d_data, int* d_src, int* d_weight, int *d_rev_meta,bool *d_modified_next,bool* d_propBw,bool* d_visitBw,bool* d_propFw,bool* d_visitFw,int* d_scc,int* d_range){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  if (d_scc[src] == -1){ // if filter begin 
...
__global__ void vHong_kernel(int V, int E, int* d_meta, int* d_data, int* d_src, int* d_weight, int *d_rev_meta,bool *d_modified_next,int* d_scc,bool* d_visitFw,bool* d_visitBw,int* d_range,bool* d_propBw,bool* d_propFw){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  if (d_scc[src] == -1){ // if filter begin 
    if (d_visitFw[src] == false || d_visitBw[src] == false){ // if filter begin 
      ...
      int newRange = 3 * d_range[src] + ext; // DEVICE ASSTMENT in .h
