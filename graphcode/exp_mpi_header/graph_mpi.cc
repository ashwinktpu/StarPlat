...
void freeAllUpdateMemory (std::vector<int*> updates) {
  ...
  Graph::Graph(char* file_name, boost::mpi::communicator world , int32_t undirected, bool optimized)
  {
    ...
    FILE* graph_file = fileIO (file_name) ;  // deduce the file name .
    ...
    csrConverter csr (residual_graph, this->global_to_local, this->local_vertices, this->local_edges, world.rank () ) ;
    ...
    this->global_to_local = global_to_local ;
