...
)
{
  ...
  //Reading kernel file
  FILE* kernelfp = fopen("sssp_dsl_prev.cl", "rb"); 
  size_t program_size;
  ...
  while(!(finished)){
    ...
    cl_kernel Compute_SSSP_prev = clCreateKernel(program, "Compute_SSSP_prev_kernel", &status);
    if(status != CL_SUCCESS){
      printf("Failed to create Compute_SSSP_prev kernel.\n");
      return ;
    }
    status = clSetKernelArg(Compute_SSSP_prev,0, sizeof(int),(void *)&V);
    status = clSetKernelArg(Compute_SSSP_prev,1, sizeof(int),(void *)&E);
    status = clSetKernelArg(Compute_SSSP_prev,2, sizeof(cl_mem),(void *)&d_meta);
    status = clSetKernelArg(Compute_SSSP_prev,3, sizeof(cl_mem),(void *)&d_data);
    status = clSetKernelArg(Compute_SSSP_prev,4, sizeof(cl_mem),(void *)&d_src);
    status = clSetKernelArg(Compute_SSSP_prev,5, sizeof(cl_mem),(void *)&d_weight);
    status = clSetKernelArg(Compute_SSSP_prev,6, sizeof(cl_mem),(void *)&d_rev_meta);
    status = clSetKernelArg(Compute_SSSP_prev,7, sizeof(cl_mem),(void *)&d_finished);
    status = clSetKernelArg(Compute_SSSP_prev,8, sizeof(cl_mem),(void *)&d_modified_next);
    status = clSetKernelArg(Compute_SSSP_prev, 9, sizeof(cl_mem), (void *)&d_modified);
    status = clSetKernelArg(Compute_SSSP_prev, 10, sizeof(cl_mem), (void *)&d_weight);
    status = clSetKernelArg(Compute_SSSP_prev, 11, sizeof(cl_mem), (void *)&d_dist);

    local_size = 128;
    global_size = (V + local_size -1)/ local_size * local_size;

    status = clEnqueueNDRangeKernel(command_queue,Compute_SSSP_prev, 1,NULL, &global_size, &local_size , 0,NULL,&event);
    clWaitForEvents(1,&event);
    status = clGetEventProfilingInfo(event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
    status = clGetEventProfilingInfo(event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
    kernelTime = (double)(end-start)/convertToMS;
    ...
    status = clReleaseKernel(Compute_SSSP_prev);
