#include "libOpenCL.h"
__kernel void initBC_kernel( __global float *d_BC , float value, int v){
  unsigned int id = get_global_id(0);
  if(id<v){
    d_BC[id] = value;
  }
}
__kernel void initdelta_kernel( __global float *d_delta , float value, int v){
  unsigned int id = get_global_id(0);
  if(id<v){
    d_delta[id] = value;
  }
...
__global__ void fwd_pass(int V, __global int* d_meta, __global int* d_data, __global int* d_weight, __global float* d_delta, __global double* d_sigma, __global int* d_level, __global int* d_hops_from_source, __global int* d_finished
  ,__global float* d_BC)
{
