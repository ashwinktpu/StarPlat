...
void Compute_BC(graph& g,float* BC,std::set<int>& sourceSet)
{
  ...
  cl_mem d_BC = clCreateBuffer(context,CL_MEM_READ_WRITE,(V)*sizeof(float),NULL, &status);


  //BEGIN DSL PARSING 

  //Reading kernel file
  FILE* kernelfp = fopen("bc_dsl_v2.cl", "rb"); 
  size_t program_size;
  ...
  for(itr=sourceSet.begin();itr!=sourceSet.end();itr++) 
  {
    ...
    float *h_delta = (float *)malloc(V*sizeof(float));
    cl_mem d_delta = clCreateBuffer(context,CL_MEM_READ_WRITE,(V)*sizeof(float),NULL, &status);
    ...
    *hop_from_source = (*hop_from_source)-1;
    ...
    while(*hops_from_source > 1)
    {
      ...
      status = clSetKernelArg(back_pass_kernel, 4, sizeof(cl_mem), (void *)&d_delta);
      status = clSetKernelArg(back_pass_kernel, 5, sizeof(cl_mem), (void *)&d_sigma);
      status = clSetKernelArg(back_pass_kernel, 6, sizeof(cl_mem), (void *)&d_level);
      status = clSetKernelArg(back_pass_kernel, 7, sizeof(cl_mem), (void *)&d_hops_from_source);
      status = clSetKernelArg(back_pass_kernel, 8, sizeof(cl_mem), (void *)&d_finished);
  ...
  clEnqueueReadBuffer(command_queue, d_BC , CL_TRUE, 0, sizeof(float)*V, BC, 0, NULL, NULL );
