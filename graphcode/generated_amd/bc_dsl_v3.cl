...
__global__ void fwd_pass(int V, __global int* d_meta, __global int* d_data, __global int* d_weight, __global int* d_level, __global int* d_hops_from_source, __global int* d_finished,__global double* d_sigma,__global double* d_BC)
{
  unsigned int v = get_global_id(0);
  if(v>= V) return ;
  if(d_level[v]== (*d_hop_from_source))
  {
...
__global__ void back_pass(int V, __global int* d_meta, __global int* d_data, __global int* d_weight, __global int* d_level, __global int* d_hops_from_source, __global int* d_finished,__global double* d_sigma,__global double* d_delta,__global double* d_BC)
{
  unsigned int v = get_global_id(0);
  if(v>= V) return ;
  if(d_levle[v]==(*d_hop_from_source)-1)
  {
