...
__global__ void incHop(int* d_hops_from_source) {
  ...
  __global__ void bc_backward_pass(int* d_offset,int* d_edgeList,int* d_edgeLen, double* d_sigma, double* d_delta, double* d_BC, int* d_level, int* d_hops_from_source, unsigned n) {
    // TO replace - DONE
    // u --> v
    // v --> w
...
void Compute_BC(graph &G, std::set<unsigned> sourceSet, bool printAns=false) {
