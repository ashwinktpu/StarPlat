...
__global__ void vHong_kernel11(int start,int end,int V, int E, int* d_meta, int* d_data, int* d_weight,int* d_src,int* d_rev_meta,int* d_scc,int* d_outDeg,int* d_inDeg,int* d_pivotField,int* d_range,bool* d_fpoint5){ // BEGIN KER FUN via ADDKERNEL
  unsigned src = blockIdx.x * blockDim.x + threadIdx.x;
  int num_vertices = end-start;
  if( src < num_vertices) {
    src+=start;
