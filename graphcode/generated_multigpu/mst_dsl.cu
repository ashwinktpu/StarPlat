...

{
  ...
  int* h_nodeId_temp1 = (int*)malloc((V+1)*(devicecount)*sizeof(int));
  cudaSetDevice(0);
  int* d_nodeId_temp1;
  cudaMalloc(&d_nodeId_temp1,(V+1)*(devicecount)*sizeof(int));
  int* d_nodeId_temp2;
  cudaMalloc(&d_nodeId_temp2,(V+1)*(devicecount)*sizeof(int));
  int** h_color;
  ...
  int* h_color_temp1 = (int*)malloc((V+1)*(devicecount)*sizeof(int));
  cudaSetDevice(0);
  int* d_color_temp1;
  cudaMalloc(&d_color_temp1,(V+1)*(devicecount)*sizeof(int));
  int* d_color_temp2;
  cudaMalloc(&d_color_temp2,(V+1)*(devicecount)*sizeof(int));
  bool** h_isMSTEdge;
  ...
  bool* h_isMSTEdge_temp1 = (bool*)malloc((V+1)*(devicecount)*sizeof(bool));
  cudaSetDevice(0);
  bool* d_isMSTEdge_temp1;
  cudaMalloc(&d_isMSTEdge_temp1,(V+1)*(devicecount)*sizeof(bool));
  bool* d_isMSTEdge_temp2;
  cudaMalloc(&d_isMSTEdge_temp2,(V+1)*(devicecount)*sizeof(bool));
  for(int i=0;i<devicecount;i++)
  {
    cudaSetDevice(i);
  ...
  while(!noNewComp) {
    ...
    int** h_minEdge;
    h_minEdge = (int**)malloc(sizeof(int*)*(devicecount+1));
    for(int i=0;i<=devicecount;i++){
      h_minEdge[i]=(int*)malloc(sizeof(int)*(V+1));
    }
    int** d_minEdge;
    ...
    int* h_minEdge_temp1 = (int*)malloc((V+1)*(devicecount)*sizeof(int));
    cudaSetDevice(0);
    int* d_minEdge_temp1;
    cudaMalloc(&d_minEdge_temp1,(V+1)*(devicecount)*sizeof(int));
    int* d_minEdge_temp2;
    cudaMalloc(&d_minEdge_temp2,(V+1)*(devicecount)*sizeof(int));
    for(int i=0;i<devicecount;i++)
    {
      cudaSetDevice(i);
    ...
    for(int i=0;i<devicecount;i++){
      cudaSetDevice(i);
      cudaMemcpyAsync(h_minEdge[i],d_minEdge[i],(V+1)*sizeof(int),cudaMemcpyDeviceToHost);
      cudaDeviceSynchronize();
    }
    for(int i=0;i<devicecount;i++)
    {
      cudaSetDevice(i);
      Boruvka_kernel2<<<numBlocks, threadsPerBlock>>>(h_vertex_partition[i],h_vertex_partition[i+1],V,E,d_offset[i],d_edges[i],d_weight[i],d_src[i],d_rev_meta[i],d_minEdge[i],d_color[i],d_noNewComp[i]);
    }
    ...
    for(int i=0;i<devicecount;i++){
      cudaSetDevice(i);
      cudaMemcpyAsync(h_minEdge_temp1+i*(V+1),d_minEdge[i],sizeof(int)*(V+1),cudaMemcpyDeviceToHost);
      cudaDeviceSynchronize();
    }
    cudaSetDevice(0);
    cudaMemcpy(d_minEdge_temp1,h_minEdge_temp1,(V+1)*(devicecount)*sizeof(int),cudaMemcpyHostToDevice);
    for(int i=0;i<devicecount;i++){
      cudaMemcpy(d_minEdge_temp2+i*(V+1),h_minEdge[i],sizeof(int)*(V+1),cudaMemcpyHostToDevice);
    }
    Compute_correct<int><<<numBlocks,threadsPerBlock>>>(d_minEdge_temp1,d_minEdge_temp2,V,devicecount);
    for(int i=0;i<devicecount;i++){
      cudaMemcpy(h_minEdge[i],d_minEdge_temp1+i*(V+1),(V+1)*sizeof(int),cudaMemcpyDeviceToHost);
    }
    for(int i=0;i<devicecount;i++){
      cudaSetDevice(i);
      cudaMemcpyAsync(d_minEdge[i],h_minEdge[i],(V+1)*sizeof(int),cudaMemcpyHostToDevice);
    }
    for(int i=0;i<devicecount;i++){
      cudaSetDevice(i);
      cudaDeviceSynchronize();
    ...
    int** h_minEdgeOfComp;
    h_minEdgeOfComp = (int**)malloc(sizeof(int*)*(devicecount+1));
    for(int i=0;i<=devicecount;i++){
      h_minEdgeOfComp[i]=(int*)malloc(sizeof(int)*(V+1));
    }
    int** d_minEdgeOfComp;
    ...
    for(int i=0;i<devicecount;i++){
      cudaSetDevice(i);
      cudaMemcpyAsync(h_minEdgeOfComp[i],d_minEdgeOfComp[i],(V+1)*sizeof(int),cudaMemcpyDeviceToHost);
      cudaDeviceSynchronize();
    }
    ...
    while(!finishedMinEdge) {
      ...
      {
        cudaSetDevice(i);
        Boruvka_kernel3<<<numBlocks, threadsPerBlock>>>(h_vertex_partition[i],h_vertex_partition[i+1],V,E,d_offset[i],d_edges[i],d_weight[i],d_src[i],d_rev_meta[i],d_minEdgeOfComp[i],d_minEdge[i],d_color[i],d_finishedMinEdge[i]);
      }
      ...
      for(int i=0;i<devicecount;i++){
        cudaMemcpy(d_minEdgeOfComp_temp2+i*(V+1),h_minEdgeOfComp[i],sizeof(int)*(V+1),cudaMemcpyHostToDevice);
      }
      Compute_correct<int><<<numBlocks,threadsPerBlock>>>(d_minEdgeOfComp_temp1,d_minEdgeOfComp_temp2,V,devicecount);
      for(int i=0;i<devicecount;i++){
        cudaMemcpy(h_minEdgeOfComp[i],d_minEdgeOfComp_temp1+i*(V+1),(V+1)*sizeof(int),cudaMemcpyDeviceToHost);
      }
      for(int i=0;i<devicecount;i++){
        cudaSetDevice(i);
        cudaMemcpyAsync(d_minEdgeOfComp[i],h_minEdgeOfComp[i],(V+1)*sizeof(int),cudaMemcpyHostToDevice);
      }
      for(int i=0;i<devicecount;i++){
        cudaSetDevice(i);
        cudaDeviceSynchronize();
    ...
    {
      cudaSetDevice(i);
      Boruvka_kernel4<<<numBlocks, threadsPerBlock>>>(h_vertex_partition[i],h_vertex_partition[i+1],V,E,d_offset[i],d_edges[i],d_weight[i],d_src[i],d_rev_meta[i],d_color[i],d_nodeId[i],d_minEdgeOfComp[i]);
    }
    ...
    {
      cudaSetDevice(i);
      Boruvka_kernel5<<<numBlocks, threadsPerBlock>>>(h_vertex_partition[i],h_vertex_partition[i+1],V,E,d_offset[i],d_edges[i],d_weight[i],d_src[i],d_rev_meta[i],d_color[i],d_nodeId[i],d_minEdgeOfComp[i],d_isMSTEdge[i]);
    }
    ...
    for(int i=0;i<devicecount;i++){
      cudaSetDevice(i);
      cudaMemcpyAsync(h_isMSTEdge_temp1+i*(V+1),d_isMSTEdge[i],sizeof(bool)*(V+1),cudaMemcpyDeviceToHost);
      cudaDeviceSynchronize();
    }
    cudaSetDevice(0);
    cudaMemcpy(d_isMSTEdge_temp1,h_isMSTEdge_temp1,(V+1)*(devicecount)*sizeof(bool),cudaMemcpyHostToDevice);
    for(int i=0;i<devicecount;i++){
      cudaMemcpy(d_isMSTEdge_temp2+i*(V+1),h_isMSTEdge[i],sizeof(bool)*(V+1),cudaMemcpyHostToDevice);
    }
    Compute_correct<bool><<<numBlocks,threadsPerBlock>>>(d_isMSTEdge_temp1,d_isMSTEdge_temp2,V,devicecount);
    for(int i=0;i<devicecount;i++){
      cudaMemcpy(h_isMSTEdge[i],d_isMSTEdge_temp1+i*(V+1),(V+1)*sizeof(bool),cudaMemcpyDeviceToHost);
    }
    for(int i=0;i<devicecount;i++){
      cudaSetDevice(i);
      cudaMemcpyAsync(d_isMSTEdge[i],h_isMSTEdge[i],(V+1)*sizeof(bool),cudaMemcpyHostToDevice);
    }
    for(int i=0;i<devicecount;i++){
      cudaSetDevice(i);
      cudaDeviceSynchronize();
    ...
    {
      cudaSetDevice(i);
      Boruvka_kernel6<<<numBlocks, threadsPerBlock>>>(h_vertex_partition[i],h_vertex_partition[i+1],V,E,d_offset[i],d_edges[i],d_weight[i],d_src[i],d_rev_meta[i],d_color[i],d_nodeId[i],d_minEdgeOfComp[i],d_noNewComp[i]);
    }
    ...
    for(int i=0;i<devicecount;i++){
      cudaSetDevice(i);
      cudaMemcpyAsync(h_color_temp1+i*(V+1),d_color[i],sizeof(int)*(V+1),cudaMemcpyDeviceToHost);
      cudaDeviceSynchronize();
    }
    cudaSetDevice(0);
    cudaMemcpy(d_color_temp1,h_color_temp1,(V+1)*(devicecount)*sizeof(int),cudaMemcpyHostToDevice);
    for(int i=0;i<devicecount;i++){
      cudaMemcpy(d_color_temp2+i*(V+1),h_color[i],sizeof(int)*(V+1),cudaMemcpyHostToDevice);
    }
    Compute_correct<int><<<numBlocks,threadsPerBlock>>>(d_color_temp1,d_color_temp2,V,devicecount);
    for(int i=0;i<devicecount;i++){
      cudaMemcpy(h_color[i],d_color_temp1+i*(V+1),(V+1)*sizeof(int),cudaMemcpyDeviceToHost);
    }
    for(int i=0;i<devicecount;i++){
      cudaSetDevice(i);
      cudaMemcpyAsync(d_color[i],h_color[i],(V+1)*sizeof(int),cudaMemcpyHostToDevice);
    }
    for(int i=0;i<devicecount;i++){
      cudaSetDevice(i);
      cudaDeviceSynchronize();
    ...
    while(!finished) {
      ...
      {
        cudaSetDevice(i);
        Boruvka_kernel7<<<numBlocks, threadsPerBlock>>>(h_vertex_partition[i],h_vertex_partition[i+1],V,E,d_offset[i],d_edges[i],d_weight[i],d_src[i],d_rev_meta[i],d_color[i],d_finished[i]);
      }
