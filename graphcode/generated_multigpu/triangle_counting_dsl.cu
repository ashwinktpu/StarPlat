...

{
  ...
  for(int i=0;i<=devicecount;i+=1){
    h_triangle_count[i] = (long*)malloc(sizeof(long));
  }

  long** d_triangle_count;
  d_triangle_count = (long**)malloc(sizeof(long*)*devicecount);
  for(int i = 0 ; i < devicecount ; i++){
    cudaSetDevice(i);
  ...
  for(int i=0;i<devicecount;i++){
    cudaSetDevice(i);
    cudaMemcpyAsync(h_triangle_count[i],d_triangle_count[i],sizeof(long),cudaMemcpyDeviceToHost);
  }
  for(int i=0;i<devicecount;i++){
    cudaSetDevice(i);
    cudaDeviceSynchronize();
